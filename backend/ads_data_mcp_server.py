...
async def confirm_requirement(
    ...
    触发时机：用户说「帮我做…」「创建一个…」「新增…」「我需要…功能」「做一个系统/模块」
    不触发：用户在提问、报 Bug、描述现象、讨论方案
    !! 注意：用户明确说「创建工单」「建个工单」「加个工单」时，不要用此工具，改用 confirm_direct_ticket !!
...
async def confirm_direct_ticket(
    ...
    触发时机（任意满足一条）：
      - 用户明确说「创建工单」「建个工单」「加个工单」「帮我建一个工单」（含"工单"二字）
      - 用户说「修复 X」「加一个 X」「调整 X」「改一下 X」「更新 X」
      - 描述非常具体，明确知道改哪个文件/功能
      - 预计工作量 ≤ 4 小时，单人单次完成
