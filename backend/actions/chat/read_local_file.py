...
FILE_IO_TIMEOUT = 15  # 秒，网络盘/慢速盘超时保护
    ...
    async def run(self, context: Dict[str, Any]) -> ActionResult:
        ...
        try:
            content = await asyncio.wait_for(
                loop.run_in_executor(None, _read_file_sync, path),
                timeout=FILE_IO_TIMEOUT,
            )
        except asyncio.TimeoutError:
            logger.warning("读取文件超时(%ds): %s", FILE_IO_TIMEOUT, path)
            return ActionResult(success=False, error=f"读取超时（>{FILE_IO_TIMEOUT}s），文件可能在慢速网络盘上: {path}")
