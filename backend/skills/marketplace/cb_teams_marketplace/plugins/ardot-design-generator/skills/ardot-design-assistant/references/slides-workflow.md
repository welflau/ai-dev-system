...
- **`create_design`** — Create a new blank file and open it. Optionally accepts a `filename`. **Wait for a context update confirming it is ready** before proceeding.
- **`open_design`** — Open an existing file by URL or file ID. **Wait for a context update confirming it is ready** before proceeding.
...
1. Call `fetch_style_guide_tags` to list available tags
