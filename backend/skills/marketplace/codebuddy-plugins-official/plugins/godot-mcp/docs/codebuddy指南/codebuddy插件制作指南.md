...
检查结构：确保目录在插件根目录，而不是在 .codebuddy-plugin/ 内
单独测试组件：分别检查每个命令、代理和钩子
使用调试模式：用 --debug 参数启动 CodeBuddy 查看详细日志
plugin.json 清单格式
...
添加文档：包含一个 README.md，说明安装和使用方式
版本管理：在 plugin.json 中使用语义化版本
创建或使用市场：通过插件市场分发
与他人测试：在更广泛分发之前，让团队成员测试插件
...
最佳实践
插件开发建议
遵循命名规范：使用清晰、描述性的插件名称（kebab-case，无空格）
提供完整元数据：在 plugin.json 中提供详细的描述和作者信息
版本管理：使用语义化版本号（Semantic Versioning）
文档完善：为每个命令和技能提供清晰的描述
测试充分：使用 --plugin-dir 在本地测试插件后再发布
安全注意事项
