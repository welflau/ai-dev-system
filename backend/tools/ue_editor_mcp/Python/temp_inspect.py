...
def find_node(n, name):
    if n and n.get('name') == name: return n
    for c in (n.get('children', []) if n else []):
        f = find_node(c, name)
        if f: return f
    return None

for name in ['MainMenuBottomBar', 'MainMenuButtonContainer', 'HostButtons_Row', 'ServerBrowserButtons_Row']:
    node = find_node(res.get('tree', {}), name)
    print(f"=== {name} ===")
