...
def register_all_actions(registry: ActionRegistry) -> None:
    ...
    registry.register_many(_OFFICIAL_TOOLSET_ACTIONS)
...
# =========================================================================
# Official UE ToolsetRegistry Bridge
# =========================================================================
_OFFICIAL_TOOLSET_ACTIONS = [
    ActionDef(
        id="list_toolsets",
        command="list_toolsets",
        tags=("official", "toolset", "mcp", "discover", "read"),
        description="Official UE MCP-compatible toolset discovery. Lists ToolsetRegistry toolsets available in the running editor.",
    ...
    ActionDef(
        id="describe_toolset",
        command="describe_toolset",
        tags=("official", "toolset", "mcp", "schema", "read"),
        description="Official UE MCP-compatible toolset schema lookup. Use after list_toolsets to inspect tool names and input schemas.",
        input_schema={
            "type": "object",
            "properties": {
                "toolset_name": {"type": "string", "description": "Name of the toolset to describe. Use list_toolsets first."},
            },
            "required": ["toolset_name"],
        },
        capabilities=("read",),
        examples=({"toolset_name": "EditorToolset"},),
    ),
    ActionDef(
        id="call_tool",
        command="call_tool",
        tags=("official", "toolset", "mcp", "execute"),
        description=(
            "Official UE MCP-compatible tool dispatcher. Calls a ToolsetRegistry tool by toolset_name/tool_name. "
            "Short calls return inline; long-running calls return a job_id immediately so the LLM can poll tool_job_status."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "toolset_name": {"type": "string", "description": "Name of the ToolsetRegistry toolset containing the tool."},
                "tool_name": {"type": "string", "description": "Name of the tool inside the toolset. Use describe_toolset first."},
                "arguments": {"type": "object", "additionalProperties": True, "description": "Arguments matching the tool schema. Defaults to {}."},
                "wait_seconds": {"type": "number", "description": "Inline wait budget before returning a job_id for polling. Default: 0.15."},
            },
            "required": ["toolset_name", "tool_name"],
        },
        examples=(
            {"toolset_name": "EditorToolset", "tool_name": "GetSelectedAssets", "arguments": {}},
            {"toolset_name": "AutomationTestToolset", "tool_name": "RunTests", "arguments": {"test_names": []}, "wait_seconds": 0.0},
        ),
    ),
    ActionDef(
        id="tool_job_status",
        command="tool_job_status",
        tags=("official", "toolset", "mcp", "job", "poll", "read"),
        description="Poll the result of a long-running official ToolsetRegistry call returned by call_tool.",
        input_schema={
            "type": "object",
            "properties": {
                "job_id": {"type": "string", "description": "Job id returned by call_tool."},
            },
            "required": ["job_id"],
        },
        capabilities=("read",),
        examples=({"job_id": "00000000-0000-0000-0000-000000000000"},),
    ),
]
...
_EDITOR_ACTIONS = [
    ...
    ActionDef(
        ...
        input_schema={
            "type": "object",
            "properties": {
                "level_path": {"type": "string", "description": "Long package name or object path, e.g. '/Game/P110_2/TPS/Maps/L_TPS_Test'"},
                "if_exists": {"type": "string", "enum": ["error", "overwrite", "skip", "reuse"], "description": "Existing asset behavior (default: error)"},
                "open_level": {"type": "boolean", "description": "Open the new/reused level after creation (default: true)"},
                "save_dirty": {"type": "boolean", "description": "Save dirty packages before replacing the current editor map (default: false)"},
            },
            "required": ["level_path"],
        },
        capabilities=("write",),
        risk="moderate",
        examples=({"level_path": "/Game/P110_2/TPS/Maps/L_TPS_Test", "if_exists": "overwrite"},),
    ),
    ActionDef(
        ...
        input_schema={
            "type": "object",
            "properties": {
                "source_asset_path": {"type": "string", "description": "Source asset package or object path"},
                "destination_asset_path": {"type": "string", "description": "Destination package or object path"},
                "if_exists": {"type": "string", "enum": ["error", "overwrite", "skip", "reuse"], "description": "Existing asset behavior (default: error)"},
                "save": {"type": "boolean", "description": "Save the duplicated package after creation (default: true)"},
            },
    ...
    ActionDef(
        ...
        tags=("editor", "assets", "rename", "move", "dryrun", "plan", "referencers", "conflict", "read"),
        description=(
            "Dry-run one or more asset moves/renames. Reports source validity, destination conflicts, "
            "duplicate destinations, protected paths, and external referencers without modifying assets."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "old_asset_path": {"type": "string", "description": "Single mode: source object path or package path"},
                "new_package_path": {"type": "string", "description": "Single mode: destination parent path, e.g. /Game/UI"},
                "new_name": {"type": "string", "description": "Single mode: destination asset name"},
                "items": {
                    "type": "array",
                    "description": "Batch mode: list of planned rename operations",
                    "items": {
                        "type": "object",
                        "properties": {
                            "old_asset_path": {"type": "string"},
                ...
                "include_referencers": {"type": "boolean", "description": "Include external referencer summaries (default: true)"},
                "max_referencers_per_item": {"type": "integer", "description": "Maximum referencer paths returned per item (default: 20)"},
                "allow_protected_paths": {
                    "type": "boolean",
                    "description": "Allow non-/Game paths and World Partition external paths (default: false)",
                },
            },
        },
        capabilities=("read",),
        ...
        examples=(
            {
                "items": [
                    {
                        "old_asset_path": "/Game/P110_2/Blueprints/BP_Old.BP_Old",
                        "new_package_path": "/Game/P110_2/Blueprints",
                        "new_name": "BP_New",
                    }
                ],
    ...
    ActionDef(
        ...
        tags=("editor", "assets", "delete", "remove", "dryrun", "referencers", "cleanup"),
        description=(
            "Delete one or more assets with a default dry-run, protected-path checks, map guards, "
            "and optional external referencer blocking."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "asset_path": {"type": "string", "description": "Single asset object path or package path"},
                "asset_paths": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Batch asset paths",
                },
                "dry_run": {"type": "boolean", "description": "Preview only; default true"},
                "require_unreferenced": {"type": "boolean", "description": "Block assets with external referencers; default true"},
                "force": {"type": "boolean", "description": "Allow deletion despite external referencers; default false"},
                "allow_map_assets": {"type": "boolean", "description": "Allow World/umap assets; default false"},
                "allow_protected_paths": {
                    "type": "boolean",
                    "description": "Allow non-/Game paths and World Partition external paths; default false",
                },
                "max_referencers_per_asset": {"type": "integer", "description": "Maximum referencer paths returned per asset (default: 20)"},
            },
        },
        capabilities=("write", "destructive"),
        risk="destructive",
        examples=(
            {
                "asset_paths": ["/Game/P110_2/Temp/T_Obsolete.T_Obsolete"],
                "dry_run": True,
    ...
    ActionDef(
        ...
        tags=("editor", "assets", "directory", "folder", "delete", "empty", "cleanup", "dryrun"),
        description=(
            "Delete empty /Game directories only. Scans AssetRegistry and disk, processes bottom-up, "
            "and defaults to dry-run."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": "Single Content Browser directory, e.g. /Game/P110_2/Temp"},
                "paths": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Batch directories",
                },
                "dry_run": {"type": "boolean", "description": "Preview only; default true"},
                "recursive": {"type": "boolean", "description": "Scan child directories bottom-up; default true"},
                "allow_protected_paths": {
                    "type": "boolean",
                    "description": "Allow /Game root, non-/Game, or World Partition external paths; default false",
                },
                "max_directories": {"type": "integer", "description": "Safety cap for candidate directories (default: 1000)"},
            },
        },
        capabilities=("write", "destructive"),
        risk="moderate",
    ...
    ActionDef(
        ...
        tags=("editor", "assets", "manifest", "rename", "move", "cleanup", "redirector", "dryrun", "llm"),
        description=(
            "Validate or apply an LLM-produced asset maintenance manifest. "
            "Plan mode returns normalized operations and a plan_hash; apply requires confirm_plan_hash before writes."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "mode": {"type": "string", "enum": ["plan", "dry_run", "apply"]},
                ...
                "manifest": {
                    ...
                    "properties": {
                        ...
                        "cleanup": {
                            "type": "object",
                            "properties": {
                                "empty_directories": {"type": "array"},
                            },
                        },
                    },
                    "required": ["schema"],
                },
                "include_referencers": {"type": "boolean"},
                "max_referencers_per_item": {"type": "integer"},
                "allow_protected_paths": {"type": "boolean"},
                "allow_case_only_renames": {"type": "boolean"},
                "min_confidence": {"type": "number"},
                "auto_fixup_redirectors": {"type": "boolean"},
                "fixup_mode": {"type": "string", "enum": ["delete", "leave", "prompt"]},
                "delete_empty_directories": {"type": "boolean"},
                "apply_empty_directory_cleanup": {"type": "boolean"},
                    ...
                    "items": [
                        {
                            ...
                            "source": {"object_path": "/Game/P110_2/Blueprints/BP_Old.BP_Old"},
                            "target": {"directory": "/Game/P110_2/Blueprints", "asset_name": "BP_New"},
                        }
                    ],
                },
            },
