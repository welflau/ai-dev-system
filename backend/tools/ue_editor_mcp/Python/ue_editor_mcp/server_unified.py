...
Also exposes official UE MCP-compatible ToolsetRegistry meta-tools:
    list_toolsets, describe_toolset, call_tool, tool_job_status
...
_UMG_COMPONENT_TYPE_MAP: dict[str, tuple[str, str, str | None]] = {
    ...
    "CommonButtonBase": ("add_generic_widget_to_widget", "component_name", "CommonButtonBase"),
    "CommonTextBlock": ("add_generic_widget_to_widget", "component_name", "CommonTextBlock"),
    "UserWidget": ("add_generic_widget_to_widget", "component_name", None),
...
TOOLS = [
    ...
    Tool(
        ...
        description=(
            ...
            "Returns compact results by default to save tokens; set result_verbosity='full' only when details are needed. "
            "Supports cross-step refs in params, e.g. {'$ref':'steps[0].path'} or '$ref:steps[0].path'. "
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "result_verbosity": {
                    "type": "string",
                    "enum": ["compact", "full"],
                    "description": "compact (default) returns token-light per-step summaries and failures[]; full returns complete sub-results",
                },
            },
            "required": ["actions"],
        },
    ...
    Tool(
        name="ue_logs_tail",
        description="Tail recent logs. 'source' selects: 'python' (MCP command log, max 200), 'editor' (UE editor log via C++ ring buffer, capacity 10000), or 'both'.",
        inputSchema={
            "type": "object",
            "properties": {
                "n": {
                    "type": "integer",
                    "description": "Number of log entries to return (default: 20). For 'python' source max=200; for 'editor' source max=10000 (C++ ring buffer capacity).",
                },
                "source": {
                    "type": "string",
    ...
    Tool(
        name="list_toolsets",
        description=(
            "Official UE MCP-compatible ToolsetRegistry discovery. "
            "Use this to list available official UE toolsets before describe_toolset."
        ),
        inputSchema={"type": "object", "properties": {}},
    ),
    Tool(
        ...
        description=(
            "Official UE MCP-compatible ToolsetRegistry schema lookup. "
            "Use after list_toolsets to inspect tool names and input schemas."
        ),
        inputSchema={
            "type": "object",
            "properties": {
                "toolset_name": {
                    "type": "string",
                    "description": "Name of the toolset to describe. Use list_toolsets to discover names.",
                },
            },
    ...
    Tool(
        name="call_tool",
        description=(
            "Official UE MCP-compatible ToolsetRegistry dispatcher. "
            "Calls a tool by toolset_name/tool_name with schema-matching arguments. "
            "Short calls return inline; true long-running calls return job_id immediately so the LLM can poll tool_job_status instead of waiting on the MCP request."
        ),
        inputSchema={
            "type": "object",
            "properties": {
                "toolset_name": {
                    "type": "string",
                    "description": "Name of the toolset containing the tool. Required for the UEEditorMCP bridge.",
                },
                "tool_name": {
                    "type": "string",
                    "description": "Name of the tool inside the toolset. Use describe_toolset to discover tool names.",
                },
                "arguments": {
                    "type": "object",
                    "additionalProperties": True,
                    "description": "Arguments matching the target tool schema. Defaults to {}.",
                },
                "wait_seconds": {
                    "type": "number",
                    "description": "Inline wait budget before returning a job_id for polling. Default: 0.15; use 0 for known long tasks.",
                },
            },
    ...
    Tool(
        ...
        description=(
            "Poll a long-running official ToolsetRegistry call returned by call_tool. "
            "Call with job_id until status is completed or failed."
        ),
        inputSchema={
            "type": "object",
            "properties": {
...
def _handle_tool(name: str, args: dict) -> Any:
    ...
    if name in ("list_toolsets", "describe_toolset", "call_tool", "tool_job_status"):
        return _send_command(name, args or None)
    ...
    if name == "ue_batch":
        ...
        result_verbosity = args.get("result_verbosity", "compact")
        ...
        for i, item in enumerate(actions):
            ...
            cpp_commands.append({"type": command, "params": final_params or {}, "action_id": aid})
        ...
        result = _send_command("batch_execute", {
            ...
            "result_verbosity": result_verbosity,
        })
        elapsed = (time.perf_counter() - t0) * 1000
        ...
        return {
            ...
            "has_failures": result.get("has_failures", result.get("failed", 0) > 0),
            "first_failure_index": result.get("first_failure_index"),
            "result_verbosity": result.get("result_verbosity", result_verbosity),
    ...
    if name == "ue_logs_tail":
        # python_log limited by python ring (_LOG_BUFFER_SIZE=200);
        # editor_log limited only by C++ FMCPLogCapture::BufferCapacity (10000) — see EditorActions.cpp.
        n_raw = int(args.get("n", 20))
        n_python = min(n_raw, _LOG_BUFFER_SIZE)
        n_editor = min(max(1, n_raw), 10000)
        ...
        if source in ("python", "both"):
            py_entries = list(_command_log)[-n_python:]
            result["python_log"] = {
        ...
        if source in ("editor", "both"):
            editor_params: dict[str, Any] = {"count": n_editor}
