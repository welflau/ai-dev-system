...
- 当前可检索 Action 数：**142**（运行时 `ue_actions_search` 预期，重启 MCP 后生效）。
...
- 新增 `node.bind_uobject_delegate` Action（2026-05-09）：从已有节点的 UObject 输出 pin（如 `GetSession.ReturnValue`）解析目标类，查找其 `BlueprintAssignable` 多播委托，创建 `UK2Node_AddDelegate` 与签名匹配的 `UK2Node_CustomEvent`，并自动连接对象 pin 与 delegate pin。用于第三方 SDK/Subsystem/异步代理对象的回调绑定，例如 `P111SessionAgent.OnCreateResult`。
...
- **LLM 资产维护 Manifest**（2026-07-09）：新增 `editor.process_asset_maintenance_manifest` Action，支持版本化 manifest 干跑、稳定 `plan_hash`、hash 确认 apply、重命名/移动预检、redirector fixup 调度与重命名后空目录清理。
...
- **Phase 6 完成**（2026-02-23）：PIE 控制（启动/停止/状态查询）、日志清空与会话分段、断言型验证工具、World Outliner 管理（Actor 重命名/文件夹组织/选择/层级查询）。新增 9 个 Action；截至 `node.bind_uobject_delegate` 补齐后，总计 142 个。
...
VS Code / MCP Client (GitHub Copilot Chat)
  ...
  server_unified.py  (单进程, 7 MCP tools)
        ...
        │     ├── 142 action 定义 (id, tags, schema, examples)
...
| 节点创建 | `node.*` | `node.add_event`, `node.add_function_call`, `node.bind_uobject_delegate` |
