 ...
 *
 * Reconstructs every node in a Blueprint using Unreal's native
 * FBlueprintEditorUtils::RefreshAllNodes flow. This is the programmatic
 * equivalent of Blueprint Editor > File > Refresh All Nodes and preserves
 * compatible links/default values while repairing stale component/function
 * references.
 *
 * Parameters:
 *   - blueprint_name (required): Name of the Blueprint to refresh
 *
 * Returns:
 *   - name: Blueprint name
 *   - refreshed_node_count: Number of Blueprint graph nodes processed
 *   - status: Blueprint status after reconstruction
 */
class UEEDITORMCP_API FRefreshBlueprintNodesAction : public FBlueprintAction
{
public:
	virtual TSharedPtr<FJsonObject> ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context) override;
...
protected:
	virtual bool Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError) override;
	virtual FString GetActionName() const override { return TEXT("refresh_blueprint_nodes"); }
 ...
 *
 * Rebinds stale Blueprint graph references after an SCS component has been
 * deleted and recreated with the same variable name. The repair uses the new
 * SCS variable GUID, reconstructs component variable/event nodes in place, and
 * preserves compatible pins and links.
 *
 * Parameters:
 *   - blueprint_name (required): Name of the Blueprint to repair
 *   - component_name (required): SCS component variable name
 *
 * Returns:
 *   - variable_nodes_rebound: Number of Get/Set variable nodes repaired
 *   - bound_events_rebound: Number of component-bound event nodes repaired
 *   - function_nodes_rebound: Number of stale component function calls repaired
 *   - orphaned_pins_removed: Number of unlinked obsolete pins removed
 *   - orphaned_pin_defaults_migrated: Number of obsolete defaults copied to replacement pins
 *   - component_variable_guid: Current SCS variable GUID
 *   - status: Blueprint status after compilation
 */
class UEEDITORMCP_API FRepairBlueprintComponentReferencesAction : public FBlueprintAction
{
public:
	virtual TSharedPtr<FJsonObject> ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context) override;
...
protected:
	virtual bool Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError) override;
	virtual FString GetActionName() const override { return TEXT("repair_blueprint_component_references"); }
};
 ...
 *
 * Removes an SCS component owned by the target Blueprint.
 *
 * Parameters:
 *   - blueprint_name (required): Name of the target Blueprint
 *   - component_name (required): Component variable name to remove
 *
 * Returns:
 *   - component_name: Removed component name
 *   - component_type: Removed component class name
 */
class UEEDITORMCP_API FRemoveComponentFromBlueprintAction : public FBlueprintAction
{
public:
	virtual TSharedPtr<FJsonObject> ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context) override;
...
protected:
	virtual bool Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError) override;
	virtual FString GetActionName() const override { return TEXT("remove_component_from_blueprint"); }
};
 ...
 * Sets skeletal mesh, anim class, material, and relative transform on a SkeletalMeshComponent.
 * Supports inherited Character Mesh by falling back to the Blueprint CDO component instance.
 */
class UEEDITORMCP_API FSetSkeletalMeshPropertiesAction : public FBlueprintAction
{
public:
	virtual TSharedPtr<FJsonObject> ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context) override;
...
protected:
	virtual bool Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError) override;
	virtual FString GetActionName() const override { return TEXT("set_skeletal_mesh_properties"); }
};
