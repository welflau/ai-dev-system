	...
	const TSharedPtr<FJsonValue>& Value, FString& OutErrorMessage)
{
	...
	{
		// FText 默认本地化为空，使用 FText::FromString 形成 culture-invariant 文本，
		// 适合通过 MCP 设定占位/调试文案；正式本地化文本应在编辑器内通过 StringTable 配置。
	...
	else if (FStructProperty* StructProp = CastField<FStructProperty>(Property))
	{
		...
		else if (StructProp->Struct == TBaseStructure<FVector2D>::Get())
		{
			if (Value->Type == EJson::Array)
			{
				...
				if (Arr.Num() >= 2)
				{
					FVector2D* V2 = (FVector2D*)PropertyAddr;
					V2->X = Arr[0]->AsNumber();
					V2->Y = Arr[1]->AsNumber();
					return true;
				}
		...
		else if (StructProp->Struct == TBaseStructure<FIntPoint>::Get())
		{
			if (Value->Type == EJson::Array)
			{
				TArray<TSharedPtr<FJsonValue>> Arr = Value->AsArray();
				if (Arr.Num() >= 2)
				{
					FIntPoint* P = (FIntPoint*)PropertyAddr;
					P->X = static_cast<int32>(Arr[0]->AsNumber());
					P->Y = static_cast<int32>(Arr[1]->AsNumber());
					return true;
				}
			}
		}
	...
	{
		// TSubclassOf<UFoo>; accept full class path like
		// "/Game/Path/MyBP.MyBP_C" or "/Script/Engine.Actor".
		FString ClassPath = Value->AsString();

		auto TryResolveClass = [](const FString& InPath) -> UClass*
		{
			if (InPath.IsEmpty())
			{
				return nullptr;
			}
			// 1) Direct LoadClass (works for /Script/... and many already-loaded BPs).
			if (UClass* C = LoadClass<UObject>(nullptr, *InPath))
			{
				return C;
			}
			// 2) LoadObject<UClass> covers SoftClassPath-style strings too.
			if (UClass* C = LoadObject<UClass>(nullptr, *InPath))
			{
				return C;
			}
			// 3) Blueprint asset path (no _C); load asset, return GeneratedClass.
			//    e.g. "/Game/Path/Foo.Foo" or "/Game/Path/Foo.Foo_C" -> strip _C, load asset.
			FString AssetPath = InPath;
			if (AssetPath.EndsWith(TEXT("_C")))
			{
				AssetPath.LeftChopInline(2, EAllowShrinking::No);
			}
		...
		UClass* LoadedClass = TryResolveClass(ClassPath);

		if (LoadedClass)
		{
			// Validate against MetaClass to avoid type mismatch crashes downstream.
			UClass* MetaClass = ClassProp->MetaClass;
			if (MetaClass && !LoadedClass->IsChildOf(MetaClass))
			{
				OutErrorMessage = FString::Printf(TEXT("Class '%s' is not a subclass of expected '%s'"),
		...
		// LoadClass failed: try the generic ImportText fallback which knows
		// how to resolve "/Game/.../Foo.Foo_C" via UClass::ImportText.
		const TCHAR* CursorClass = *ClassPath;
		const TCHAR* ResultClass = Property->ImportText_Direct(CursorClass, PropertyAddr,
			Object, PPF_None, GLog);
	...
	else if (FSoftClassProperty* SoftClassProp = CastField<FSoftClassProperty>(Property))
	{
		...
		if (LoadedClass)
		{
			SoftClassProp->SetPropertyValue(PropertyAddr, FSoftObjectPtr(LoadedClass));
			return true;
		}
		OutErrorMessage = FString::Printf(TEXT("Could not load soft class: %s"), *ClassPath);
		return false;
	}
	else if (FObjectProperty* ObjectProp = CastField<FObjectProperty>(Property))
	{
		// Generic UObject*: load asset by path.
		FString ObjPath = Value->AsString();
		UObject* LoadedObject = LoadObject<UObject>(nullptr, *ObjPath);
		if (LoadedObject)
		{
			// Validate by property class.
			if (ObjectProp->PropertyClass && !LoadedObject->IsA(ObjectProp->PropertyClass))
			{
				OutErrorMessage = FString::Printf(TEXT("Object '%s' is not a %s"),
		...
		OutErrorMessage = FString::Printf(TEXT("Could not load object: %s"), *ObjPath);
		return false;
	}

	// ----- Generic ImportText fallback -----
	// For complex types (FMapProperty / FSetProperty / FArrayProperty / nested struct,
	// or ClassProperty/SoftObjectProperty cases that need UE's text-import resolution)
	// fall back to FProperty::ImportText_Direct using the JSON value as a UE textual literal.
	if (Value.IsValid() && Value->Type == EJson::String)
	{
		const FString TextLiteral = Value->AsString();
		const TCHAR* CursorPtr = *TextLiteral;
		const TCHAR* Result = Property->ImportText_Direct(CursorPtr, PropertyAddr,
			/*OwnerObject=*/Object, PPF_None, GLog);
		if (Result != nullptr)
		{
			return true;
		}
