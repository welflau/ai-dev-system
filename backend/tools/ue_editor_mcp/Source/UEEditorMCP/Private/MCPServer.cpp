...
bool FMCPClientHandler::ReceiveMessage(FString& OutMessage)
{
	...
	// Convert to string. 注意：Length 是 UTF-8 字节数，而 FString(Count, TCHAR*) 把 Count 当 wide char 个数；
	// 多字节字符（中文等）会让 wide char 数量 < Length，旧写法会越界读垃圾导致 JSON 解析失败。
	// 这里用以 NUL 结尾的临时缓冲先 NUL terminate，再用 FString 隐式从 const ANSICHAR*/UTF8 构造。
	Buffer.Add(0); // NUL terminator
