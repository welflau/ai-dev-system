...
void FMCPEditorContext::SaveDirtyPackages()
{
	...
	for (UPackage* Package : DirtyPackages)
	{
		...
		// Pass the package's primary asset for both maps and ordinary assets.
		// A null top-level asset can report success while failing to persist newly
		// created Blueprint SCS nodes reliably across an editor restart.
...
{
	if (BlueprintName == TEXT("__LEVEL_BLUEPRINT__"))
	{
		UWorld* EditorWorld = GEditor ? GEditor->GetEditorWorldContext().World() : nullptr;
		ULevel* PersistentLevel = EditorWorld ? EditorWorld->PersistentLevel : nullptr;
		return PersistentLevel ? PersistentLevel->GetLevelScriptBlueprint() : nullptr;
	}
