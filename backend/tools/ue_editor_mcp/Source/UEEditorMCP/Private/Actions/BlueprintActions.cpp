...
TSharedPtr<FJsonObject> FCreateBlueprintAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	FString IfExists = GetOptionalString(Params, TEXT("if_exists"), TEXT("error"));
	...
	const bool bAssetExists = UEditorAssetLibrary::DoesAssetExist(FullAssetPath) || FindPackage(nullptr, *FullAssetPath) != nullptr;
	if (bAssetExists)
	{
		...
		else if (IfExists.Equals(TEXT("skip"), ESearchCase::IgnoreCase) || IfExists.Equals(TEXT("reuse"), ESearchCase::IgnoreCase))
		{
			...
			{
				if (UPackage* ExistingPackage = FindPackage(nullptr, *FullAssetPath))
				{
					ExistingBlueprint = FindObject<UBlueprint>(ExistingPackage, *BlueprintName);
				}
			}
			if (!ExistingBlueprint)
			{
				return CreateErrorResponse(
					FString::Printf(TEXT("An asset already exists at '%s' but it is not a Blueprint"), *FullAssetPath),
					TEXT("asset_exists_type_mismatch"));
			}

			Context.SetCurrentBlueprint(ExistingBlueprint);
			TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
			Result->SetStringField(TEXT("name"), BlueprintName);
			Result->SetStringField(TEXT("path"), FullAssetPath);
			Result->SetStringField(TEXT("parent_class"), ExistingBlueprint->GeneratedClass && ExistingBlueprint->GeneratedClass->GetSuperClass() ? ExistingBlueprint->GeneratedClass->GetSuperClass()->GetName() : TEXT("Unknown"));
		...
		{
			return CreateErrorResponse(
				FString::Printf(TEXT("An asset already exists at '%s'. Set if_exists to overwrite, skip, or reuse."), *FullAssetPath),
				TEXT("asset_exists"));
		}
	}
...
UClass* FCreateBlueprintAction::ResolveParentClass(const FString& ParentClassName) const
{
	...
	// 通用回退：上面的 LoadClass<AActor> 只能解析 Actor 派生类，无法解析非 Actor 的原生反射类
	// （例如 UGameplayEffect / UGameplayAbility 派生的项目类，如 GE_BaseEffect）。
	// 这里改用 FindFirstObject<UClass> 按精确名查找任意已加载模块中的反射类，不限定基类。
	for (const FString& Name : NameVariants)
	{
		if (UClass* FoundClass = FindFirstObject<UClass>(*Name, EFindFirstObjectOptions::ExactClass))
		{
			return FoundClass;
		}
	}
...
// ============================================================================
// FRefreshBlueprintNodesAction
// ============================================================================

bool FRefreshBlueprintNodesAction::Validate(
...
TSharedPtr<FJsonObject> FRefreshBlueprintNodesAction::ExecuteInternal(
	const TSharedPtr<FJsonObject>& Params,
	FMCPEditorContext& Context)
{
	UBlueprint* Blueprint = GetTargetBlueprint(Params, Context);
...
// ============================================================================
// FRepairBlueprintComponentReferencesAction
// ============================================================================

bool FRepairBlueprintComponentReferencesAction::Validate(
	const TSharedPtr<FJsonObject>& Params,
	FMCPEditorContext& Context,
	FString& OutError)
{
	...
	{
		OutError = TEXT("component_name must not be empty");
		return false;
	}
...
TSharedPtr<FJsonObject> FRepairBlueprintComponentReferencesAction::ExecuteInternal(
	const TSharedPtr<FJsonObject>& Params,
	FMCPEditorContext& Context)
{
	UBlueprint* Blueprint = GetTargetBlueprint(Params, Context);
	...
	{
		return CreateErrorResponse(TEXT("Blueprint or SimpleConstructionScript not found"), TEXT("invalid_blueprint"));
	}
	...
	// Regenerate the skeleton first so the recreated SCS component has a real
	// reflected member property before graph nodes are reconstructed.
	FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(Blueprint);

	FObjectProperty* ComponentProperty = nullptr;
	if (Blueprint->SkeletonGeneratedClass)
	{
		ComponentProperty = FindFProperty<FObjectProperty>(Blueprint->SkeletonGeneratedClass, ComponentFName);
	}
	if (!ComponentProperty && Blueprint->GeneratedClass)
	{
		ComponentProperty = FindFProperty<FObjectProperty>(Blueprint->GeneratedClass, ComponentFName);
	}
	if (!ComponentProperty)
	{
		return CreateErrorResponse(
			FString::Printf(
				TEXT("Component '%s' exists in SCS but no reflected member property was generated"),
				*ComponentName),
			TEXT("component_property_not_generated"));
	}

	int32 VariableNodesRebound = 0;
	int32 BoundEventsRebound = 0;
	int32 FunctionNodesRebound = 0;
	int32 OrphanedPinsRemoved = 0;
	int32 OrphanedPinDefaultsMigrated = 0;
	...
	{
		if (VariableNode && VariableNode->GetVarName() == ComponentFName)
		{
			VariableNode->Modify();
			VariableNode->VariableReference.SetSelfMember(ComponentFName, ComponentNode->VariableGuid);
			VariableNode->ReconstructNode();
			++VariableNodesRebound;
		}
	}

	UClass* ComponentClass = ComponentNode->ComponentTemplate->GetClass();
	TArray<UK2Node_ComponentBoundEvent*> BoundEventNodes;
	FBlueprintEditorUtils::GetAllNodesOfClass(Blueprint, BoundEventNodes);
	for (UK2Node_ComponentBoundEvent* EventNode : BoundEventNodes)
	{
		if (!EventNode || EventNode->ComponentPropertyName != ComponentFName)
		{
			continue;
		}

		FMulticastDelegateProperty* DelegateProperty =
			FindFProperty<FMulticastDelegateProperty>(ComponentClass, EventNode->DelegatePropertyName);
		if (!DelegateProperty)
		{
			continue;
		}
	...
	// When a component is deleted, Refresh All Nodes may degrade member calls
	// into unresolved self-context calls and orphan their Target pins. Restore
	// only unresolved functions that genuinely exist on this component class.
	TArray<UK2Node_CallFunction*> FunctionNodes;
	FBlueprintEditorUtils::GetAllNodesOfClass(Blueprint, FunctionNodes);
	for (UK2Node_CallFunction* FunctionNode : FunctionNodes)
	{
		if (!FunctionNode || FunctionNode->GetTargetFunction())
		{
			continue;
		}

		const FName FunctionName = FunctionNode->FunctionReference.GetMemberName();
		UFunction* ComponentFunction = ComponentClass->FindFunctionByName(FunctionName);
		...
		FunctionNode->Modify();
		FunctionNode->SetFromFunction(ComponentFunction);
		FunctionNode->ReconstructNode();
		++FunctionNodesRebound;
	}

	// Migrate a non-default value from an obsolete unlinked pin to a newly
	// reconstructed pin with the same member name, then remove the obsolete pin.
	// This mirrors the editor's "Reset orphaned pin" cleanup while retaining
	// settings across signature type metadata changes (for example enum pins).
	for (UK2Node_CallFunction* FunctionNode : FunctionNodes)
	{
		if (!FunctionNode)
		{
		...
		TArray<UEdGraphPin*> OrphanedPins;
		for (UEdGraphPin* Pin : FunctionNode->Pins)
		{
			if (Pin && Pin->bOrphanedPin && Pin->LinkedTo.IsEmpty())
			{
				OrphanedPins.Add(Pin);
			}
		}

		const UEdGraphSchema* Schema =
			FunctionNode->GetGraph() ? FunctionNode->GetGraph()->GetSchema() : nullptr;
		for (UEdGraphPin* OrphanedPin : OrphanedPins)
		{
			UEdGraphPin* ReplacementPin = nullptr;
			for (UEdGraphPin* CandidatePin : FunctionNode->Pins)
			{
				if (CandidatePin &&
					!CandidatePin->bOrphanedPin &&
					CandidatePin->PinName == OrphanedPin->PinName &&
					CandidatePin->Direction == OrphanedPin->Direction)
				{
					ReplacementPin = CandidatePin;
					break;
				}
			}

			if (ReplacementPin && Schema && !OrphanedPin->DoesDefaultValueMatchAutogenerated())
			{
				Schema->TrySetDefaultValue(*ReplacementPin, OrphanedPin->DefaultValue);
				if (ReplacementPin->DefaultValue == OrphanedPin->DefaultValue)
				{
					++OrphanedPinDefaultsMigrated;
				}
			}

			FunctionNode->Modify();
			if (FunctionNode->RemovePin(OrphanedPin))
			{
				++OrphanedPinsRemoved;
			}
		}
	...
	// Reconstruct dependent function-call nodes after their target variable pins
	// have regained the component class type, then perform a full compile.
	FBlueprintEditorUtils::RefreshAllNodes(Blueprint);
	FKismetEditorUtilities::CompileBlueprint(Blueprint);
	Blueprint->MarkPackageDirty();
	Context.SetCurrentBlueprint(Blueprint);
	...
	Result->SetStringField(TEXT("component_variable_guid"), ComponentNode->VariableGuid.ToString(EGuidFormats::DigitsWithHyphens));
	Result->SetNumberField(TEXT("variable_nodes_rebound"), VariableNodesRebound);
	Result->SetNumberField(TEXT("bound_events_rebound"), BoundEventsRebound);
	Result->SetNumberField(TEXT("function_nodes_rebound"), FunctionNodesRebound);
	Result->SetNumberField(TEXT("orphaned_pins_removed"), OrphanedPinsRemoved);
	Result->SetNumberField(TEXT("orphaned_pin_defaults_migrated"), OrphanedPinDefaultsMigrated);
	Result->SetStringField(TEXT("status"), UEnum::GetValueAsString(Blueprint->Status));
	...
	UE_LOG(
		LogMCP,
		Log,
		TEXT("UEEditorMCP: Rebound component '%s' in Blueprint '%s' (%d variable nodes, %d bound events, %d function calls, %d orphaned pins removed, status=%s)"),
		*ComponentName,
		*Blueprint->GetName(),
		VariableNodesRebound,
		BoundEventsRebound,
...
bool FAddComponentToBlueprintAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	...
	if (Params->HasField(TEXT("parent_component")))
	{
		FString ParentComponentName;
		if (!Params->TryGetStringField(TEXT("parent_component"), ParentComponentName) || ParentComponentName.IsEmpty())
		{
			OutError = TEXT("parent_component must be a non-empty component name when specified");
			return false;
		}
	}
...
TSharedPtr<FJsonObject> FAddComponentToBlueprintAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	// Use UE's SubobjectDataSubsystem rather than manipulating SCS arrays
	// directly. This is the same editor path used by the Components panel and
	// guarantees that the new component node/template serialize across restart.
	USubobjectDataSubsystem* SubobjectSubsystem = USubobjectDataSubsystem::Get();
	if (!SubobjectSubsystem)
	{
		return CreateErrorResponse(TEXT("SubobjectDataSubsystem is unavailable"), TEXT("subsystem_unavailable"));
	}

	if (!Blueprint->GeneratedClass)
	{
		return CreateErrorResponse(TEXT("Blueprint has no generated class"), TEXT("invalid_blueprint"));
	}

	TArray<FSubobjectDataHandle> SubobjectHandles;
	SubobjectSubsystem->GatherSubobjectData(Blueprint->GeneratedClass->GetDefaultObject(), SubobjectHandles);
	if (SubobjectHandles.IsEmpty())
	{
		return CreateErrorResponse(TEXT("Blueprint subobject hierarchy is empty"), TEXT("subobject_hierarchy_empty"));
	}

	const FString ParentComponentName = GetOptionalString(Params, TEXT("parent_component"), TEXT(""));
	...
	if (!ParentComponentName.IsEmpty())
	{
		for (const FSubobjectDataHandle& Handle : SubobjectHandles)
		{
			...
			if (Data && Data->GetVariableName() == FName(*ParentComponentName))
			{
				ParentHandle = Handle;
				break;
			}
	...
	else
	{
		for (const FSubobjectDataHandle& Handle : SubobjectHandles)
		{
			ParentHandle = USubobjectDataSubsystem::GetActorRootHandle(Handle);
			if (ParentHandle.IsValid())
			{
				break;
			}
	...
	if (!ParentHandle.IsValid())
	{
		return CreateErrorResponse(TEXT("Failed to resolve Blueprint actor root"), TEXT("actor_root_not_found"));
	}
	...
	AddParams.bConformTransformToParent = false;

	TSet<USCS_Node*> ExistingNodes;
	...
	FText AddFailureReason;
	FSubobjectDataHandle NewHandle = SubobjectSubsystem->AddNewSubobject(AddParams, AddFailureReason);
	if (!NewHandle.IsValid())
	{
		return CreateErrorResponse(
			AddFailureReason.IsEmpty() ? TEXT("Failed to add Blueprint subobject") : AddFailureReason.ToString(),
			TEXT("creation_failed"));
	}

	FSubobjectData* NewData = NewHandle.GetData();
	USCS_Node* NewNode = nullptr;
	for (USCS_Node* CandidateNode : Blueprint->SimpleConstructionScript->GetAllNodes())
	{
		if (CandidateNode && !ExistingNodes.Contains(CandidateNode))
		{
	...
	if (!NewNode)
	{
		return CreateErrorResponse(TEXT("Created subobject has no SCS node"), TEXT("creation_failed"));
	}

	if (NewNode->GetVariableName() != FName(*ComponentName))
	{
		if (!SubobjectSubsystem->RenameSubobject(NewHandle, FText::FromString(ComponentName)))
		{
			return CreateErrorResponse(
				FString::Printf(TEXT("Failed to rename component to '%s'"), *ComponentName),
				TEXT("rename_failed"));
		}
		NewData = NewHandle.GetData();
		if (!NewData || NewData->GetVariableName() != FName(*ComponentName))
		{
			return CreateErrorResponse(TEXT("Renamed subobject did not retain the requested variable name"), TEXT("rename_failed"));
		}
	}
	...
	if (UNiagaraComponent* NiagaraComponent = Cast<UNiagaraComponent>(NewNode->ComponentTemplate))
	{
		const FString NiagaraSystemPath = GetOptionalString(Params, TEXT("niagara_system"), TEXT(""));
		if (!NiagaraSystemPath.IsEmpty())
		{
			UNiagaraSystem* NiagaraSystem = Cast<UNiagaraSystem>(UEditorAssetLibrary::LoadAsset(NiagaraSystemPath));
			if (!NiagaraSystem)
			{
				return CreateErrorResponse(
					FString::Printf(TEXT("Niagara system not found: %s"), *NiagaraSystemPath),
	...
	// Adding an SCS component changes the generated class layout. Regenerate the
	// skeleton before compiling so graph nodes can immediately resolve the new
	// component member property (including components recreated under an old name).
	FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(Blueprint);
	...
	// Persist the SCS mutation here with the Blueprint itself as the top-level
	// asset. Generic dirty-package save paths may select a generated class (or
	// no top-level object), which can report success without serializing a newly
	// inserted root SCS node across editor restart.
	Blueprint->SimpleConstructionScript->Modify();
	Blueprint->MarkPackageDirty();
	bool bSaved = false;
	FString PackageFileName;
	...
	Result->SetNumberField(TEXT("scs_node_count"), Blueprint->SimpleConstructionScript->GetAllNodes().Num());
	return CreateSuccessResponse(Result);
}
...
UClass* FAddComponentToBlueprintAction::ResolveComponentClass(const FString& ComponentTypeName) const
{
	...
	// Global fallback: search any loaded UClass by short name and verify it derives from UActorComponent.
	// This allows custom project component classes to be resolved without hard-coding their /Script/<Module> path.
	for (const FString& Candidate : Candidates)
	{
		FString PathName = Candidate;
		if (Candidate.StartsWith(TEXT("U")))
		...
		if (UClass* Found = FindFirstObject<UClass>(*PathName, EFindFirstObjectOptions::ExactClass | EFindFirstObjectOptions::EnsureIfAmbiguous))
		{
...
// ============================================================================
// FRemoveComponentFromBlueprintAction
// ============================================================================

bool FRemoveComponentFromBlueprintAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	if (!ValidateBlueprint(Params, Context, OutError))
	{
		return false;
	...
	FString ComponentName;
	return GetRequiredString(Params, TEXT("component_name"), ComponentName, OutError);
}

TSharedPtr<FJsonObject> FRemoveComponentFromBlueprintAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	UBlueprint* Blueprint = GetTargetBlueprint(Params, Context);
	if (!Blueprint || !Blueprint->SimpleConstructionScript)
	{
	...
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Component '%s' not found on Blueprint '%s'"), *ComponentName, *Blueprint->GetName()),
			TEXT("component_not_found")
		);
	}

	UActorComponent* ComponentTemplate = ComponentNode->ComponentTemplate;
	const FString ComponentType = ComponentTemplate ? ComponentTemplate->GetClass()->GetName() : TEXT("Unknown");
	...
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Removed component '%s' but Blueprint compile failed: %s"), *ComponentName, *CompileError),
			TEXT("compile_failed")
		);
	}

	UE_LOG(LogMCP, Log, TEXT("UEEditorMCP: Removed component '%s' (%s) from Blueprint '%s'"),
		*ComponentName, *ComponentType, *Blueprint->GetName());
...
// ============================================================================
// FSetSkeletalMeshPropertiesAction
// ============================================================================

bool FSetSkeletalMeshPropertiesAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	if (!ValidateBlueprint(Params, Context, OutError))
	{
		return false;
...
TSharedPtr<FJsonObject> FSetSkeletalMeshPropertiesAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	UBlueprint* Blueprint = GetTargetBlueprint(Params, Context);
	FString Error;
	FString ComponentName;
	...
	FString ComponentSource = TEXT("scs");

	if (USCS_Node* ComponentNode = FMCPCommonUtils::FindComponentNode(Blueprint, ComponentName))
	{
		MeshComponent = Cast<USkeletalMeshComponent>(ComponentNode->ComponentTemplate);
	}

	if (!MeshComponent && Blueprint && Blueprint->GeneratedClass)
	{
		AActor* CDO = Cast<AActor>(Blueprint->GeneratedClass->GetDefaultObject());
		if (ACharacter* CharacterCDO = Cast<ACharacter>(CDO))
		{
			if (ComponentName == TEXT("Mesh") || ComponentName == CharacterCDO->GetMesh()->GetName() || ComponentName == CharacterCDO->GetMesh()->GetReadableName())
			{
				MeshComponent = CharacterCDO->GetMesh();
				ComponentSource = TEXT("character_cdo");
			}
		}
		if (!MeshComponent && CDO)
		{
			TArray<USkeletalMeshComponent*> Components;
			CDO->GetComponents<USkeletalMeshComponent>(Components);
			for (USkeletalMeshComponent* Candidate : Components)
			{
				...
				const FString CandidateReadableName = Candidate->GetReadableName();
				if (CandidateName == ComponentName || CandidateReadableName == ComponentName)
				{
	...
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("SkeletalMeshComponent not found: %s"), *ComponentName),
			TEXT("component_not_found")
		);
	}

	FString MeshPath = GetOptionalString(Params, TEXT("skeletal_mesh"), TEXT(""));
	if (!MeshPath.IsEmpty())
	{
		USkeletalMesh* Mesh = Cast<USkeletalMesh>(UEditorAssetLibrary::LoadAsset(MeshPath));
		if (!Mesh)
		{
			return CreateErrorResponse(FString::Printf(TEXT("SkeletalMesh not found: %s"), *MeshPath), TEXT("asset_not_found"));
		}
		MeshComponent->SetSkeletalMesh(Mesh);
	}

	FString AnimClassPath = GetOptionalString(Params, TEXT("anim_class"), TEXT(""));
	if (!AnimClassPath.IsEmpty())
	{
		UClass* AnimClass = LoadClass<UAnimInstance>(nullptr, *AnimClassPath);
		if (!AnimClass)
		{
			if (UAnimBlueprint* AnimBlueprint = Cast<UAnimBlueprint>(UEditorAssetLibrary::LoadAsset(AnimClassPath)))
			{
				AnimClass = AnimBlueprint->GeneratedClass;
			}
		}
		if (!AnimClass)
		{
			return CreateErrorResponse(FString::Printf(TEXT("Anim class not found: %s"), *AnimClassPath), TEXT("asset_not_found"));
		}
	...
	if (!MaterialPath.IsEmpty())
	{
		...
		if (!Material)
		{
			return CreateErrorResponse(FString::Printf(TEXT("Material not found: %s"), *MaterialPath), TEXT("asset_not_found"));
		}
		MeshComponent->SetMaterial(0, Material);
	}
	...
	{
		MeshComponent->SetRelativeLocation(FMCPCommonUtils::GetVectorFromJson(Params, TEXT("relative_location")));
	}
	if (Params->HasField(TEXT("relative_rotation")))
	{
		MeshComponent->SetRelativeRotation(FMCPCommonUtils::GetRotatorFromJson(Params, TEXT("relative_rotation")));
	}
	if (Params->HasField(TEXT("relative_scale")))
	{
		MeshComponent->SetRelativeScale3D(FMCPCommonUtils::GetVectorFromJson(Params, TEXT("relative_scale")));
	}

	MarkBlueprintModified(Blueprint, Context);

	UE_LOG(LogMCP, Log, TEXT("UEEditorMCP: Set skeletal mesh properties on '%s' in Blueprint '%s' via %s"),
		*ComponentName, *Blueprint->GetName(), *ComponentSource);

	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
	Result->SetStringField(TEXT("component"), ComponentName);
	Result->SetStringField(TEXT("source"), ComponentSource);
	Result->SetStringField(TEXT("skeletal_mesh"), MeshComponent->GetSkeletalMeshAsset() ? MeshComponent->GetSkeletalMeshAsset()->GetPathName() : FString());
	return CreateSuccessResponse(Result);
}
