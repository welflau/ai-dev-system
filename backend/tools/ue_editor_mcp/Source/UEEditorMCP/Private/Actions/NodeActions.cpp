...
TSharedPtr<FJsonObject> FSetNodePinDefaultAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	// IMPORTANT (UE5 K2Schema):
	//   - Hard Object/Class pins  -> DefaultObject (UObject*), DefaultValue empty.
	//   - Soft Object/Class pins  -> DefaultValue (FSoftObjectPath string), DefaultObject must be null.
	//     Writing DefaultObject on a SoftClass/SoftObject pin will be rejected by
	//     UEdGraphSchema_K2::DefaultValueSimpleValidation with
	//     "NewDefaultObject ... specified on object pin 'X'" at compile time.
	const bool bIsHardObjectPin = TargetPin->PinType.PinCategory == UEdGraphSchema_K2::PC_Object ||
		TargetPin->PinType.PinCategory == UEdGraphSchema_K2::PC_Class ||
		TargetPin->PinType.PinCategory == UEdGraphSchema_K2::PC_Interface;
	const bool bIsSoftObjectPin = TargetPin->PinType.PinCategory == UEdGraphSchema_K2::PC_SoftObject ||
	...
	else if (bIsSoftObjectPin)
	{
		// SoftClass / SoftObject pins are stored as string SoftObjectPath in DefaultValue.
		// For SoftClass pins on UClass-derived types, the string MUST point to the
		// GeneratedClass (suffix "_C"), e.g. "/Game/.../WBP_Splash.WBP_Splash_C".
		// We auto-append "_C" for SoftClass pins if the caller forgot it.
		FString PathValue = DefaultValue;
		if (TargetPin->PinType.PinCategory == UEdGraphSchema_K2::PC_SoftClass &&
			!PathValue.IsEmpty() && !PathValue.EndsWith(TEXT("_C")))
		{
			PathValue.Append(TEXT("_C"));
		}
		TargetPin->DefaultObject = nullptr;
		TargetPin->DefaultValue = PathValue;
	}
	else
	{
...
TSharedPtr<FJsonObject> FAddBlueprintFunctionNodeAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	if (ExtraParams.IsValid())
	{
		...
		{
			const FString PinName(Pair.Key.ToView());
...
bool FAddBlueprintOverrideFunctionAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	FString FunctionName;
	if (!GetRequiredString(Params, TEXT("function_name"), FunctionName, OutError)) return false;
	return ValidateBlueprint(Params, Context, OutError);
...
TSharedPtr<FJsonObject> FAddBlueprintOverrideFunctionAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	const FString FunctionName = Params->GetStringField(TEXT("function_name"));
	const bool bCompile = GetOptionalBool(Params, TEXT("compile"), false);

	UBlueprint* Blueprint = GetTargetBlueprint(Params, Context);
	if (!Blueprint)
	{
		return CreateErrorResponse(TEXT("Blueprint is null"));
	}
	...
	for (UEdGraph* Graph : Blueprint->FunctionGraphs)
	{
		if (Graph && Graph->GetFName() == FName(*FunctionName))
		{
			...
			ResultData->SetStringField(TEXT("implementation_type"), TEXT("function_graph"));
			return CreateSuccessResponse(ResultData);
		}
	}

	UFunction* OverrideFunction = nullptr;
	UClass* OverrideFunctionClass = FBlueprintEditorUtils::GetOverrideFunctionClass(
		Blueprint,
		FName(*FunctionName),
		&OverrideFunction);

	if (!OverrideFunctionClass || !OverrideFunction)
	{
		return CreateErrorResponse(FString::Printf(
			TEXT("Override function '%s' was not found on Blueprint parent class '%s'"),
			*FunctionName,
	...
	if (!NewGraph)
	{
		return CreateErrorResponse(FString::Printf(TEXT("Failed to create override graph '%s'"), *FunctionName));
	}

	FBlueprintEditorUtils::AddFunctionGraph<UClass>(Blueprint, NewGraph, false, OverrideFunctionClass);
	...
	if (EntryNode)
	{
		Context.LastCreatedNodeId = EntryNode->NodeGuid;
	}
	...
	if (bCompile)
	{
		bCompiled = CompileBlueprint(Blueprint, CompileError);
		if (!bCompiled)
		{
			return CreateErrorResponse(FString::Printf(
				TEXT("Override graph was created, but Blueprint compilation failed: %s"),
				*CompileError), TEXT("compile_failed"));
		}
	}

	TSharedPtr<FJsonObject> ResultData = MakeShared<FJsonObject>();
	ResultData->SetBoolField(TEXT("already_exists"), false);
	ResultData->SetBoolField(TEXT("compiled"), bCompiled);
	ResultData->SetStringField(TEXT("function_name"), FunctionName);
	ResultData->SetStringField(TEXT("owner_class"), OverrideFunctionClass->GetName());
	ResultData->SetStringField(TEXT("signature_function"), OverrideFunction->GetName());
	ResultData->SetStringField(TEXT("implementation_type"), TEXT("function_graph"));
	ResultData->SetStringField(TEXT("graph_name"), NewGraph->GetName());
	if (EntryNode) ResultData->SetStringField(TEXT("entry_node_id"), EntryNode->NodeGuid.ToString());
	if (ResultNode) ResultData->SetStringField(TEXT("result_node_id"), ResultNode->NodeGuid.ToString());
...
// ============================================================================
// UObject Delegate Binding (bind BlueprintAssignable delegates on object pins)
// ============================================================================

bool FBindUObjectDelegateAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	FString TargetNodeId, TargetPinName, DelegateName;
	if (!GetRequiredString(Params, TEXT("target_node_id"), TargetNodeId, OutError)) return false;
	if (!GetRequiredString(Params, TEXT("target_pin_name"), TargetPinName, OutError)) return false;
	if (!GetRequiredString(Params, TEXT("delegate_name"), DelegateName, OutError)) return false;
	return ValidateGraph(Params, Context, OutError);
}

TSharedPtr<FJsonObject> FBindUObjectDelegateAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	FString TargetNodeId = Params->GetStringField(TEXT("target_node_id"));
	...
	if (EventName.IsEmpty())
	{
		EventName = FString::Printf(TEXT("On%s"), *DelegateName);
	}
	FVector2D Position = GetNodePosition(Params);
	...
	{
		return CreateErrorResponse(TEXT("Invalid Blueprint or target graph."));
	}

	FGuid TargetGuid;
	if (!FGuid::Parse(TargetNodeId, TargetGuid))
	{
		return CreateErrorResponse(FString::Printf(TEXT("Invalid target_node_id GUID: %s"), *TargetNodeId));
	}
	...
	if (!TargetNode)
	{
		return CreateErrorResponse(FString::Printf(TEXT("Target node not found in graph: %s"), *TargetNodeId));
	}

	UEdGraphPin* SourceObjectPin = FMCPCommonUtils::FindPin(TargetNode, TargetPinName, EGPD_Output);
	if (!SourceObjectPin)
	{
		...
		const FString PinList = OutputPins.Num() > 0 ? FString::Join(OutputPins, TEXT(", ")) : TEXT("(none)");
		return CreateErrorResponse(FString::Printf(
			TEXT("Output pin '%s' not found on target node. Available output pins: %s"),
			*TargetPinName, *PinList));
	}

	UClass* TargetClass = Cast<UClass>(SourceObjectPin->PinType.PinSubCategoryObject.Get());
	if (!TargetClass)
	{
		const UObject* SubCategoryObject = SourceObjectPin->PinType.PinSubCategoryObject.Get();
		return CreateErrorResponse(FString::Printf(
			TEXT("Pin '%s' is not a typed UObject output pin. PinCategory=%s, SubCategoryObject=%s"),
			*TargetPinName,
			*SourceObjectPin->PinType.PinCategory.ToString(),
	...
	if (!DelegateProp)
	{
		...
		const FString DelegateList = AvailableDelegates.Num() > 0
			? FString::Join(AvailableDelegates, TEXT(", "))
			: TEXT("(none)");
		return CreateErrorResponse(FString::Printf(
			TEXT("Delegate '%s' not found on UObject class '%s'. Available delegates: %s"),
			*DelegateName, *TargetClass->GetName(), *DelegateList));
	}

	if (!DelegateProp->SignatureFunction)
	{
		return CreateErrorResponse(FString::Printf(
			TEXT("Delegate '%s' on UObject class '%s' has no signature function."),
			*DelegateName, *TargetClass->GetName()));
	}

	UK2Node_AddDelegate* BindNode = FEdGraphSchemaAction_K2NewNode::SpawnNode<UK2Node_AddDelegate>(
		TargetGraph, Position, EK2NewNodeFlags::None,
		[DelegateProp, TargetClass](UK2Node_AddDelegate* Node)
		{
			Node->SetFromProperty(DelegateProp, false, TargetClass);
		}
	);
	if (!BindNode)
	{
		return CreateErrorResponse(TEXT("Failed to create UObject delegate bind node."));
	}

	FVector2D EventPosition(Position.X + 320.f, Position.Y);
	UK2Node_CustomEvent* CustomEventNode = FEdGraphSchemaAction_K2NewNode::SpawnNode<UK2Node_CustomEvent>(
		TargetGraph, EventPosition, EK2NewNodeFlags::None,
	...
	if (!CustomEventNode)
	{
		return CreateErrorResponse(TEXT("Failed to create signature-matched custom event."));
	}

	CustomEventNode->SetDelegateSignature(DelegateProp->SignatureFunction);

	const UEdGraphSchema_K2* Schema = GetDefault<UEdGraphSchema_K2>();
	UEdGraphPin* BindSelfPin = BindNode->FindPin(UEdGraphSchema_K2::PN_Self);
	if (!BindSelfPin)
	{
		BindSelfPin = BindNode->FindPin(TEXT("self"));
	}

	bool bTargetConnected = false;
	if (Schema && SourceObjectPin && BindSelfPin)
	{
		bTargetConnected = Schema->TryCreateConnection(SourceObjectPin, BindSelfPin);
	}

	UEdGraphPin* EventDelegatePin = CustomEventNode->FindPin(UK2Node_Event::DelegateOutputName);
	UEdGraphPin* BindDelegatePin = BindNode->GetDelegatePin();
	bool bDelegateConnected = false;
	if (Schema && EventDelegatePin && BindDelegatePin)
	{
		bDelegateConnected = Schema->TryCreateConnection(EventDelegatePin, BindDelegatePin);
	}

	CustomEventNode->ReconstructNode();

	MarkBlueprintModified(Blueprint, Context);
	RegisterCreatedNode(BindNode, Context);
	RegisterCreatedNode(CustomEventNode, Context);
	...
	ResultData->SetStringField(TEXT("event_node_id"), CustomEventNode->NodeGuid.ToString());
	ResultData->SetStringField(TEXT("event_name"), EventName);
	ResultData->SetStringField(TEXT("delegate_name"), DelegateName);
	ResultData->SetStringField(TEXT("target_class"), TargetClass->GetName());
	ResultData->SetBoolField(TEXT("target_connected"), bTargetConnected);
	ResultData->SetBoolField(TEXT("delegate_connected"), bDelegateConnected);

	TArray<TSharedPtr<FJsonValue>> EventPins;
	for (const UEdGraphPin* Pin : CustomEventNode->Pins)
	{
		if (Pin && Pin->Direction == EGPD_Output)
		{
			...
			PinObj->SetStringField(TEXT("name"), Pin->PinName.ToString());
			PinObj->SetStringField(TEXT("type"), Pin->PinType.PinCategory.ToString());
			EventPins.Add(MakeShared<FJsonValueObject>(PinObj));
		}
	}
	ResultData->SetArrayField(TEXT("event_output_pins"), EventPins);

	if (!bTargetConnected || !bDelegateConnected)
	{
		ResultData->SetStringField(TEXT("warning"),
			TEXT("Bind node and event were created, but one or more connections failed. Inspect target_connected and delegate_connected."));
	}
...
TSharedPtr<FJsonObject> FAddSpawnActorFromClassNodeAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	// FEdGraphSchemaAction_K2NewNode::SpawnNode can leave this specialized K2
	// node without allocated pins in a Level Script graph. Building it through
	// the normal graph-node lifecycle works for both actor and level Blueprints
	// and prevents GetClassPin()/ReconstructNode() from touching an incomplete
	// node.
	UK2Node_SpawnActorFromClass* SpawnNode =
		NewObject<UK2Node_SpawnActorFromClass>(TargetGraph);
	TargetGraph->Modify();
	TargetGraph->AddNode(SpawnNode, true, false);
	SpawnNode->CreateNewGuid();
	SpawnNode->NodePosX = static_cast<int32>(Position.X);
	SpawnNode->NodePosY = static_cast<int32>(Position.Y);
...
TSharedPtr<FJsonObject> FAddMakeStructNodeAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	else
	{
		...
		// Fallback: global search across all loaded modules (third-party plugins, project-defined structs, etc.)
		if (!ScriptStruct)
		{
			ScriptStruct = FindFirstObject<UScriptStruct>(*FullStructName, EFindFirstObjectOptions::ExactClass);
		}
		if (!ScriptStruct)
		{
			ScriptStruct = FindFirstObject<UScriptStruct>(*StructType, EFindFirstObjectOptions::ExactClass);
		}
	}
	...
	if (Params->TryGetObjectField(TEXT("pin_defaults"), PinDefaults))
	{
		for (const auto& Pair : (*PinDefaults)->Values)
		{
			...
			{
				const FString PinName(Pair.Key.ToView());
				UEdGraphPin* Pin = FMCPCommonUtils::FindPin(MakeStructNode, PinName, EGPD_Input);
				if (Pin)
				{
...
TSharedPtr<FJsonObject> FBatchSelectAndActAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	for (int32 GroupIdx = 0; GroupIdx < GroupsArray->Num(); ++GroupIdx)
	{
		...
		if (GroupObj->TryGetObjectField(TEXT("action_params"), ExtraParams) && ExtraParams && (*ExtraParams).IsValid())
		{
			for (const auto& Pair : (*ExtraParams)->Values)
			{
				ActionParams->SetField(Pair.Key.ToView(), Pair.Value);
			}
		}
...
TSharedPtr<FJsonObject> FAddAsyncActionNodeAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	if (Params->TryGetObjectField(TEXT("pin_defaults"), PinDefaults) && PinDefaults && (*PinDefaults).IsValid())
	{
		...
		{
			const FString PinName(Pair.Key.ToView());
			UEdGraphPin* Pin = FMCPCommonUtils::FindPin(AsyncNode, PinName, EGPD_Input);
			if (Pin)
			{
...
TSharedPtr<FJsonObject> FAddGenericNodeAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	// Compatibility route for MCP hosts whose long-lived Python registry has not
	// reloaded node.add_level_actor_reference yet. The dedicated action remains
	// the public API after the host restarts.
	if (SpawnerId == TEXT("__LEVEL_ACTOR_REFERENCE__"))
	{
		FString ActorName;
		FString ValidationError;
		if (!GetRequiredString(Params, TEXT("actor_name"), ActorName, ValidationError))
		{
			return CreateErrorResponse(ValidationError, TEXT("validation_failed"));
		}

		FAddLevelActorReferenceNodeAction LevelActorReferenceAction;
		return LevelActorReferenceAction.ExecuteInternal(Params, Context);
	}
	...
	if (Params->TryGetObjectField(TEXT("pin_defaults"), PinDefaults) && PinDefaults && (*PinDefaults).IsValid())
	{
		...
		{
			const FString PinName(PinPair.Key.ToView());
			UEdGraphPin* Pin = FMCPCommonUtils::FindPin(NewNode, PinName, EGPD_Input);
			if (!Pin) continue;
...
// --- add_level_actor_reference_node ---

bool FAddLevelActorReferenceNodeAction::Validate(
	...
	FString& OutError)
{
	...
	if (EditorWorld == nullptr || EditorWorld->PersistentLevel == nullptr)
	{
		OutError = TEXT("No persistent editor level is loaded");
		return false;
	}

	UBlueprint* LevelBlueprint = Context.GetBlueprintByNameOrCurrent(TEXT("__LEVEL_BLUEPRINT__"));
	if (LevelBlueprint == nullptr || !FBlueprintEditorUtils::IsLevelScriptBlueprint(LevelBlueprint))
	{
		OutError = TEXT("The current level has no valid Level Blueprint");
		return false;
	}

	const FString GraphName = GetOptionalString(Params, TEXT("graph_name"), TEXT("EventGraph"));
	return FindGraph(LevelBlueprint, GraphName, OutError) != nullptr;
}

TSharedPtr<FJsonObject> FAddLevelActorReferenceNodeAction::ExecuteInternal(
	const TSharedPtr<FJsonObject>& Params,
	FMCPEditorContext& Context)
{
	...
	FString GraphError;
	UEdGraph* TargetGraph = FindGraph(LevelBlueprint, GraphName, GraphError);
	if (TargetGraph == nullptr)
	{
		return CreateErrorResponse(GraphError, TEXT("graph_not_found"));
	}

	AActor* ExactNameMatch = nullptr;
	AActor* ExactLabelMatch = nullptr;
	...
	for (AActor* Actor : PersistentLevel->Actors)
	{
		...
		{
			ExactNameMatch = Actor;
			break;
		}

		if (Actor->GetActorLabel().Equals(ActorName, ESearchCase::CaseSensitive))
		{
			ExactLabelMatch = Actor;
			++LabelMatchCount;
		}
	}

	AActor* LevelActor = ExactNameMatch != nullptr ? ExactNameMatch : ExactLabelMatch;
	if (LevelActor == nullptr)
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Actor '%s' was not found in the persistent level"), *ActorName),
			TEXT("actor_not_found"));
	}
	if (ExactNameMatch == nullptr && LabelMatchCount > 1)
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Actor label '%s' is ambiguous; use the exact object name"), *ActorName),
			TEXT("ambiguous_actor"));
	}
	if (LevelActor->IsChildActor())
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Actor '%s' is a child actor and cannot be referenced by a Level Blueprint literal"), *ActorName),
			TEXT("unsupported_actor"));
	}
	if (FBlueprintEditorUtils::GetLevelFromBlueprint(LevelBlueprint) != LevelActor->GetLevel())
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Actor '%s' does not belong to the Level Blueprint's level"), *ActorName),
			TEXT("level_mismatch"));
	}
	...
	// Clean up a prior interrupted/legacy creation of this same reference that
	// never received a GUID. Valid existing actor-reference nodes are preserved.
	TArray<UEdGraphNode*> InvalidReferences;
	for (UEdGraphNode* ExistingNode : TargetGraph->Nodes)
	{
		UK2Node_Literal* ExistingLiteral = Cast<UK2Node_Literal>(ExistingNode);
		if (ExistingLiteral != nullptr &&
			!ExistingLiteral->NodeGuid.IsValid() &&
			ExistingLiteral->GetReferencedLevelActor() == LevelActor)
		{
			InvalidReferences.Add(ExistingLiteral);
		}
	}
	for (UEdGraphNode* InvalidReference : InvalidReferences)
	{
		TargetGraph->RemoveNode(InvalidReference);
	}

	UK2Node_Literal* LiteralNode = NewObject<UK2Node_Literal>(TargetGraph);
	TargetGraph->AddNode(LiteralNode, false, false);
	...
	LiteralNode->NodePosX = FMath::RoundToInt(Position.X);
	LiteralNode->NodePosY = FMath::RoundToInt(Position.Y);
	LiteralNode->SnapToGrid(16);

	RegisterCreatedNode(LiteralNode, Context);
	MarkBlueprintModified(LevelBlueprint, Context);

	TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
	Result->SetStringField(TEXT("node_id"), LiteralNode->NodeGuid.ToString());
	Result->SetStringField(TEXT("node_class"), LiteralNode->GetClass()->GetName());
	Result->SetStringField(TEXT("node_title"), LiteralNode->GetNodeTitle(ENodeTitleType::ListView).ToString());
	Result->SetStringField(TEXT("actor_name"), LevelActor->GetName());
	Result->SetStringField(TEXT("actor_label"), LevelActor->GetActorLabel());
	Result->SetStringField(TEXT("actor_path"), LevelActor->GetPathName());
	Result->SetStringField(TEXT("value_pin"), LiteralNode->GetValuePin()->PinName.ToString());
	Result->SetStringField(TEXT("blueprint_name"), LevelBlueprint->GetName());
