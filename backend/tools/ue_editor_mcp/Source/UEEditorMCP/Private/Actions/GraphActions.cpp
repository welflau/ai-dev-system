	...
	UEdGraph* Graph, TMap<FString, FGuid>& TempIdMap, FMCPEditorContext& Context) const
{
	...
	else if (NodeType.Equals(TEXT("FunctionCall"), ESearchCase::IgnoreCase))
	{
		...
		if (Op.Params->TryGetObjectField(TEXT("defaults"), DefaultsObj) && DefaultsObj)
		{
			...
			{
				const FString PinName(DefaultPair.Key.ToView());
				UEdGraphPin* Pin = FMCPCommonUtils::FindPin(FuncNode, PinName, EGPD_Input);
				if (Pin)
				{
					FString DefaultStr;
	...
	else if (NodeType.Equals(TEXT("VariableGet"), ESearchCase::IgnoreCase))
	{
		...
		// Optional cross-class owner (mirror VariableSet)
		FString OwnerClassName;
		UClass* OwnerClass = nullptr;
		if (Op.Params->TryGetStringField(TEXT("owner_class"), OwnerClassName) && !OwnerClassName.IsEmpty())
		{
			OwnerClass = FindFirstObject<UClass>(*OwnerClassName, EFindFirstObjectOptions::ExactClass);
			if (!OwnerClass)
			{
				OwnerClass = StaticLoadClass(UObject::StaticClass(), nullptr, *OwnerClassName);
			}
			if (!OwnerClass && OwnerClassName.Contains(TEXT("/")) && !OwnerClassName.EndsWith(TEXT("_C")))
			{
				if (UBlueprint* Bp = LoadObject<UBlueprint>(nullptr, *OwnerClassName))
				{
			...
			if (!OwnerClass)
			{
				FString ClassOnly = OwnerClassName;
				int32 DotIdx;
				if (ClassOnly.FindLastChar(TEXT('.'), DotIdx)) { ClassOnly = ClassOnly.RightChop(DotIdx + 1); }
				if (!ClassOnly.EndsWith(TEXT("_C"))) { ClassOnly += TEXT("_C"); }
				OwnerClass = FindFirstObject<UClass>(*ClassOnly, EFindFirstObjectOptions::ExactClass);
			}
			if (!OwnerClass)
			{
				Result.bSuccess = false;
				Result.Message = FString::Printf(TEXT("add_node(VariableGet): owner_class '%s' not found"), *OwnerClassName);
				return Result;
			}
		}
		...
		UK2Node_VariableGet* VarGetNode = FEdGraphSchemaAction_K2NewNode::SpawnNode<UK2Node_VariableGet>(
			Graph, Position, EK2NewNodeFlags::None,
			[VariableFName, OwnerClass](UK2Node_VariableGet* Node)
			{
				if (OwnerClass)
				{
					Node->VariableReference.SetExternalMember(VariableFName, OwnerClass);
				}
				else
				{
	...
	else if (NodeType.Equals(TEXT("VariableSet"), ESearchCase::IgnoreCase))
	{
		...
		// Optional: when the variable is declared on another class (e.g. setting
		// `SessionToJoin` on a casted UWBP_LoginServerBrowserItem_C), pass owner_class
		// so SetExternalMember is used; otherwise default to SetSelfMember.
		FString OwnerClassName;
		UClass* OwnerClass = nullptr;
		if (Op.Params->TryGetStringField(TEXT("owner_class"), OwnerClassName) && !OwnerClassName.IsEmpty())
		{
			...
			{
				Result.bSuccess = false;
				Result.Message = FString::Printf(TEXT("add_node(VariableSet): owner_class '%s' not found"), *OwnerClassName);
				return Result;
			}
		}
		...
		UK2Node_VariableSet* VarSetNode = FEdGraphSchemaAction_K2NewNode::SpawnNode<UK2Node_VariableSet>(
			Graph, Position, EK2NewNodeFlags::None,
			[VariableFName, OwnerClass](UK2Node_VariableSet* Node)
			{
				if (OwnerClass)
				{
					Node->VariableReference.SetExternalMember(VariableFName, OwnerClass);
	...
	else if (NodeType.Equals(TEXT("Cast"), ESearchCase::IgnoreCase))
	{
		...
		// Asset-path or BP class fallbacks
		if (!TargetUClass)
		{
			TargetUClass = StaticLoadClass(UObject::StaticClass(), nullptr, *ClassName);
		}
		if (!TargetUClass && ClassName.Contains(TEXT("/")) && !ClassName.EndsWith(TEXT("_C")))
		{
			if (UBlueprint* Bp = LoadObject<UBlueprint>(nullptr, *ClassName))
			{
				TargetUClass = Bp->GeneratedClass;
			}
		}
		if (!TargetUClass)
		{
			FString ClassOnly = ClassName;
			int32 DotIdx;
			if (ClassOnly.FindLastChar(TEXT('.'), DotIdx)) { ClassOnly = ClassOnly.RightChop(DotIdx + 1); }
			if (!ClassOnly.EndsWith(TEXT("_C")))
			{
				const FString WithSuffix = ClassOnly + TEXT("_C");
				TargetUClass = FindFirstObject<UClass>(*WithSuffix, EFindFirstObjectOptions::ExactClass);
			}
		}
		if (!TargetUClass)
		{
	...
	else if (NodeType.Equals(TEXT("CreateWidget"), ESearchCase::IgnoreCase))
	{
		...
		// CRITICAL: when constructing a UEdGraphNode via raw NewObject (instead of the
		// schema action helper), NodeGuid is left as zero. Initialize it explicitly so
		// that downstream RegisterCreatedNode / TempIdMap / connect ops see a valid id.
		...
		if (Op.Params->TryGetStringField(TEXT("class_name"), ClassName) && !ClassName.IsEmpty())
		{
			...
			// 1) Direct class load (works for /Script/... paths and /Game/.../X.X_C asset paths)
			WidgetClass = StaticLoadClass(UObject::StaticClass(), nullptr, *ClassName);
			// 2) Asset-style path without _C: load the UBlueprint and use its GeneratedClass
			if (!WidgetClass && ClassName.Contains(TEXT("/")) && !ClassName.EndsWith(TEXT("_C")))
			{
				if (UBlueprint* Bp = LoadObject<UBlueprint>(nullptr, *ClassName))
				{
			...
			// 3) Bare name fallback ("WBP_X" or "WBP_X_C") via global UClass search
			if (!WidgetClass)
			{
				FString ClassOnly = ClassName;
				int32 DotIdx;
				if (ClassOnly.FindLastChar(TEXT('.'), DotIdx)) { ClassOnly = ClassOnly.RightChop(DotIdx + 1); }
				if (!ClassOnly.EndsWith(TEXT("_C"))) { ClassOnly += TEXT("_C"); }
				WidgetClass = FindFirstObject<UClass>(*ClassOnly, EFindFirstObjectOptions::ExactClass);
			}
			if (WidgetClass)
			{
	...
	else if (NodeType.Equals(TEXT("K2Node_MakeArray"), ESearchCase::IgnoreCase) || NodeType.Equals(TEXT("MakeArray"), ESearchCase::IgnoreCase))
	{
		UK2Node_MakeArray* ArrayNode = FEdGraphSchemaAction_K2NewNode::SpawnNode<UK2Node_MakeArray>(
			Graph, Position, EK2NewNodeFlags::None,
			[](UK2Node_MakeArray* Node) {}
		);
		if (!ArrayNode)
		{
			Result.bSuccess = false;
			Result.Message = TEXT("Failed to create K2Node_MakeArray node");
			return Result;
		}

		int32 NumInputs = 1; // Default
		if (Op.Params->TryGetNumberField(TEXT("num_inputs"), NumInputs))
		{
			// The node might start with 1 input. We add until we reach NumInputs.
	...
	{
		Result.bSuccess = false;
		Result.Message = FString::Printf(TEXT("add_node: unsupported node_type '%s'. Supported: Event, CustomEvent, FunctionCall, Branch, Sequence, VariableGet, VariableSet, Cast, Self, Reroute, MacroInstance, CreateDelegate, Delay, CreateWidget, InputAction, FormatText, SpawnActorFromClass, MakeArray"), *NodeType);
		return Result;
	}
