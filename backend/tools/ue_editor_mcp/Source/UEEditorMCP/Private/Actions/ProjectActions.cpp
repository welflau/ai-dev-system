...
// =============================================================================
// FRemoveKeyMappingFromContextAction - Remove key from IMC
// =============================================================================
...
{
	if (!Params->HasField(TEXT("context_name")) && !Params->HasField(TEXT("context_path")))
	{
		OutError = TEXT("Missing 'context_name' or 'context_path' parameter");
		return false;
	}
	if (!Params->HasField(TEXT("key")))
	{
...
TSharedPtr<FJsonObject> FRemoveKeyMappingFromContextAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	// Resolve IMC path
	FString FullContextPath;
	if (Params->HasField(TEXT("context_path")))
	{
		FullContextPath = Params->GetStringField(TEXT("context_path"));
		// Ensure it has the asset suffix
		FString AssetName = FPaths::GetBaseFilename(FullContextPath);
		if (!FullContextPath.Contains(TEXT(".")))
		{
			FullContextPath = FullContextPath + TEXT(".") + AssetName;
		}
	}
	else
	{
		FString ContextName = Params->GetStringField(TEXT("context_name"));
		FString BasePath = TEXT("/Game/Input");
		Params->TryGetStringField(TEXT("base_path"), BasePath);
		FullContextPath = BasePath / ContextName + TEXT(".") + ContextName;
	}
	...
	FKey TargetKey{FName{*KeyStr}};

	// Optional: filter by action name
	FString FilterActionName;
	bool bFilterByAction = Params->TryGetStringField(TEXT("action_name"), FilterActionName);

	// Collect mappings to remove
	TArray<FEnhancedActionKeyMapping> MappingsToKeep;
	int32 RemovedCount = 0;

	const TArray<FEnhancedActionKeyMapping>& AllMappings = IMC->GetMappings();
	for (const FEnhancedActionKeyMapping& Mapping : AllMappings)
	{
		bool bKeyMatch = (Mapping.Key == TargetKey);
		bool bActionMatch = !bFilterByAction || (IsValid(Mapping.Action) && Mapping.Action->GetName() == FilterActionName);

		if (bKeyMatch && bActionMatch)
		{
			RemovedCount++;
		}
		else
	...
	if (RemovedCount == 0)
	{
		TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
		ResultObj->SetBoolField(TEXT("success"), true);
		ResultObj->SetStringField(TEXT("message"), FString::Printf(TEXT("No mapping found for key '%s' in IMC '%s'"), *KeyStr, *FullContextPath));
		ResultObj->SetNumberField(TEXT("removed_count"), 0);
		return ResultObj;
	}

	// Replace mappings array
	// UInputMappingContext doesn't expose a direct setter, use UnmapKey per action
	// Re-build by unmapping all and re-adding kept ones
	IMC->UnmapAll();
	for (const FEnhancedActionKeyMapping& Kept : MappingsToKeep)
	{
		if (IsValid(Kept.Action))
		{
			FEnhancedActionKeyMapping& NewMapping = IMC->MapKey(Kept.Action, Kept.Key);
			NewMapping.Modifiers = Kept.Modifiers;
			NewMapping.Triggers = Kept.Triggers;
		}
	}
	...
	ResultObj->SetNumberField(TEXT("remaining_mappings"), MappingsToKeep.Num());
	return ResultObj;
}
