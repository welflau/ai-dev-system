...
static bool ValidateParameterOverrideKeys(const TSharedPtr<FJsonObject>& Params, const FString& FieldName, FString& OutError)
{
	...
	{
		const FString ParameterName(Pair.Key.ToView());
		if (IsInvalidMaterialParameterName(ParameterName))
		{
			OutError = FString::Printf(TEXT("Invalid parameter name '%s' in '%s'. Parameter names cannot be empty or None/NAME_None."), *ParameterName, *FieldName);
			return false;
		}
	}
...
static FString GetMaterialExpressionShortName(UClass* ExpressionClass)
{
	...
	FString ShortName = ExpressionClass->GetName();
	ShortName.RemoveFromStart(TEXT("UMaterialExpression"));
	ShortName.RemoveFromStart(TEXT("MaterialExpression"));
	return ShortName;
}

static bool IsUsableMaterialExpressionClass(UClass* ExpressionClass)
{
	return ExpressionClass != nullptr
...
static void AddExpressionClassAlias(const FString& Alias, UClass* ExpressionClass)
{
	const FString Key = Alias.TrimStartAndEnd();
	if (Key.IsEmpty() || ExpressionClass == nullptr)
	{
		return;
	}

	ExpressionClassMap.FindOrAdd(Key, ExpressionClass);
}

static void RegisterExpressionClassAliases(UClass* ExpressionClass)
{
	if (!IsUsableMaterialExpressionClass(ExpressionClass))
	{
		return;
	}

	const FString ClassName = ExpressionClass->GetName();
	const FString ShortName = GetMaterialExpressionShortName(ExpressionClass);

	AddExpressionClassAlias(ClassName, ExpressionClass);
	AddExpressionClassAlias(ShortName, ExpressionClass);
	AddExpressionClassAlias(FString::Printf(TEXT("/Script/Engine.%s"), *ClassName), ExpressionClass);

	if (ShortName.EndsWith(TEXT("WS")))
	{
		AddExpressionClassAlias(ShortName.LeftChop(2), ExpressionClass);
	}

	const FString DisplayName = ExpressionClass->GetDisplayNameText().ToString();
	if (!DisplayName.IsEmpty())
	{
		AddExpressionClassAlias(DisplayName, ExpressionClass);
		AddExpressionClassAlias(DisplayName.Replace(TEXT(" "), TEXT("")), ExpressionClass);
	}
}
...
static void InitExpressionClassMap()
{
	...
	TArray<UClass*> DerivedExpressionClasses;
	GetDerivedClasses(UMaterialExpression::StaticClass(), DerivedExpressionClasses, true);
	for (UClass* ExpressionClass : DerivedExpressionClasses)
	{
		RegisterExpressionClassAliases(ExpressionClass);
	}

	for (TObjectIterator<UClass> It; It; ++It)
	{
		RegisterExpressionClassAliases(*It);
	}

	// Backward-compatible aliases and preferred short names.
...
UClass* FMaterialAction::ResolveExpressionClass(const FString& ExpressionClassName) const
{
	...
	const FString RequestedName = ExpressionClassName.TrimStartAndEnd();
	if (UClass** Found = ExpressionClassMap.Find(RequestedName))
	{
		return *Found;
	}

	for (const TPair<FString, UClass*>& Pair : ExpressionClassMap)
	{
		if (Pair.Key.Equals(RequestedName, ESearchCase::IgnoreCase))
		{
			return Pair.Value;
		}
	}

	const FString ClassName = RequestedName.StartsWith(TEXT("MaterialExpression"))
		? RequestedName
		: FString::Printf(TEXT("MaterialExpression%s"), *RequestedName);
	if (UClass* FoundClass = FindObject<UClass>(nullptr, *FString::Printf(TEXT("/Script/Engine.%s"), *ClassName)))
	{
		RegisterExpressionClassAliases(FoundClass);
		return FoundClass;
	}
...
TSharedPtr<FJsonObject> FCreateMaterialAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	const bool bAssetExists = UEditorAssetLibrary::DoesAssetExist(MaterialPackagePath) || FindPackage(nullptr, *MaterialPackagePath) != nullptr;
	if (bAssetExists)
	{
		...
		{
			UMaterial* ExistingMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset(MaterialPackagePath));
			if (!ExistingMaterial)
			{
				if (UPackage* ExistingPackage = FindPackage(nullptr, *MaterialPackagePath))
				{
					ExistingMaterial = FindObject<UMaterial>(ExistingPackage, *MaterialName);
				}
			}
			if (!ExistingMaterial)
			{
				return CreateErrorResponse(
					FString::Printf(TEXT("An asset already exists at '%s' but it is not a Material"), *MaterialPackagePath),
					TEXT("asset_exists_type_mismatch"));
			}

			Context.SetCurrentMaterial(ExistingMaterial);
		...
		{
			return CreateErrorResponse(
				FString::Printf(TEXT("An asset already exists at '%s'. Set if_exists to overwrite, skip, or reuse."), *MaterialPackagePath),
				TEXT("asset_exists"));
		}
	}
...
TSharedPtr<FJsonObject> FAddMaterialExpressionAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	{
		return CreateErrorResponse(
			FString::Printf(TEXT("Unknown expression class '%s'. Classes are discovered from UMaterialExpression reflection; use material.list_expression_classes to inspect aliases."), *ExpressionClassName),
...
// =========================================================================
// FListMaterialExpressionClassesAction
// =========================================================================

bool FListMaterialExpressionClassesAction::Validate(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context, FString& OutError)
{
	return true;
}

TSharedPtr<FJsonObject> FListMaterialExpressionClassesAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	InitExpressionClassMap();

	const FString Query = GetOptionalString(Params, TEXT("query"));
	const bool bIncludeAliases = GetOptionalBool(Params, TEXT("include_aliases"), true);

	TMap<UClass*, TArray<FString>> ClassToAliases;
	for (const TPair<FString, UClass*>& Pair : ExpressionClassMap)
	{
		if (!IsUsableMaterialExpressionClass(Pair.Value))
		{
			continue;
		}

		ClassToAliases.FindOrAdd(Pair.Value).AddUnique(Pair.Key);
	}

	TArray<UClass*> Classes;
	ClassToAliases.GetKeys(Classes);
	Classes.Sort([](const UClass& A, const UClass& B)
	{
		return GetMaterialExpressionShortName(const_cast<UClass*>(&A)) < GetMaterialExpressionShortName(const_cast<UClass*>(&B));
	});

	TArray<TSharedPtr<FJsonValue>> ClassArray;
	for (UClass* ExpressionClass : Classes)
	{
		const FString ShortName = GetMaterialExpressionShortName(ExpressionClass);
		const FString ClassName = ExpressionClass->GetName();
		const FString DisplayName = ExpressionClass->GetDisplayNameText().ToString();
		if (!Query.IsEmpty()
			&& !ShortName.Contains(Query, ESearchCase::IgnoreCase)
		...
		ClassObj->SetStringField(TEXT("display_name"), DisplayName);
		ClassObj->SetStringField(TEXT("path"), ExpressionClass->GetPathName());

		if (bIncludeAliases)
		{
			TArray<FString> Aliases = ClassToAliases.FindRef(ExpressionClass);
			Aliases.Sort();
			TArray<TSharedPtr<FJsonValue>> AliasArray;
			for (const FString& Alias : Aliases)
			{
				AliasArray.Add(MakeShared<FJsonValueString>(Alias));
			}
			ClassObj->SetArrayField(TEXT("aliases"), AliasArray);
		}

		UMaterialExpression* DefaultExpression = ExpressionClass->GetDefaultObject<UMaterialExpression>();
		int32 InputCount = 0;
		if (DefaultExpression != nullptr)
		{
			for (int32 InputIndex = 0; ; ++InputIndex)
			{
				if (DefaultExpression->GetInput(InputIndex) == nullptr)
				{
					break;
				}
				++InputCount;
			}
			ClassObj->SetNumberField(TEXT("output_count"), DefaultExpression->GetOutputs().Num());
		}
		ClassObj->SetNumberField(TEXT("input_count"), InputCount);
