...
对外暴露 **7 个固定 MCP 工具**，由后端 **动作注册表（Action Registry）** 驱动，支持 142 个动作，提供持久 TCP 连接、多客户端支持与自动保存。
...
- **143 个动作** — 蓝图、图节点、材质、UMG 控件、MVVM、增强输入、组件事件绑定、UObject 委托绑定、PIE 启停/状态查询、日志断言、Outliner 管理、资产缩略图提取、批量资产重命名及重定向修复、LLM 资产维护 manifest、与版本控制仓库的差异对比 — 均可通过动作注册表动态发现
...
| `editor.*` | 19 | 关卡 Actor、视口、资产、日志、生命周期、源码控制差异 | `editor.spawn_actor`、`editor.list_assets`、`editor.get_logs`、`editor.is_ready`、`editor.request_shutdown` |
| `layout.*` | 3 | 节点自动布局 | `layout.auto_selected`、`layout.auto_subtree`、`layout.auto_blueprint` |
| `node.*` | 20 | 蓝图图节点创建与 UObject 委托绑定 | `node.add_event`、`node.add_function_call`、`node.add_branch`、`node.bind_uobject_delegate` |
...
### UObject 委托绑定（`node.bind_uobject_delegate`）

`node.bind_uobject_delegate` 用于绑定普通 `UObject` 引用上的 `BlueprintAssignable` 多播委托。它适合第三方 SDK、Subsystem 返回的代理对象、异步任务对象等场景，例如将 `P111SDKSubsystem.GetSession().ReturnValue` 上的 `OnCreateResult` 自动绑定到一个签名匹配的 Custom Event。

该动作会自动完成：

- 根据 `target_node_id + target_pin_name` 定位已有节点的 UObject 输出 pin。
- 从 pin 类型解析目标 `UClass`。
- 在目标类上查找 `delegate_name` 对应的 `FMulticastDelegateProperty`。
- 创建 `UK2Node_AddDelegate` 绑定节点。
- 创建签名匹配的 `UK2Node_CustomEvent`。
- 连接对象引用 pin 到 bind 节点，并连接 Custom Event delegate output 到 bind 节点 delegate input。

示例：

```json
{
  "action_id": "node.bind_uobject_delegate",
  "params": {
...
返回值包含 `bind_node_id`、`event_node_id`、`target_class`、`target_connected`、`delegate_connected` 与 `event_output_pins`，便于 AI 继续把成功/失败分支接到后续蓝图逻辑。
...
- 动作注册表含 142 个 ActionDef 条目，支持关键字搜索和模式自省
...
| `Python/ue_editor_mcp/registry/actions.py` | 143 个带模式/标签/示例的 ActionDef 条目 |
...
7 工具统一 MCP 服务器，动作注册表（142 个动作）、关键字搜索、批量执行、嵌入式文档和结构化日志——**全部完成**。
