...
| 工具 | 责任 | 允许 operation |
|---|---|---|
| `memory_read` | 任务上下文、文件读取、搜索、runtime digest、上下文召回、重要记忆 | `task_context` / `get_task_context` / `get` / `search` / `search_records` / `runtime_digest` / `retrieve_context` / `important_memories` |
| `memory_write` | 写入结构化 raw 记忆与任务 checkpoint | `record` / `observation` / `checkpoint` |

`memory_read(operation="task_context")` 是唯一推荐会话开始入口。它创建/解析任务身份，返回 `context_token`，并同时返回 `active_context` 与 `current_task`。后续所有任务读写携带同一个 `context_token`。
...
| Tool | operation | 用途 |
|---|---|---|
| `memory_read` | `task_context` / `get_task_context` / `get` / `search` / `search_records` / `runtime_digest` / `retrieve_context` / `important_memories` | 读取当前任务上下文、文件读取、搜索、读 digest、召回上下文、重要记忆输出 |
...
- 主路径接入：`memory_write(distill=true)` raw 落盘后蒸馏；`memory_read(operation="retrieve_context", summarize=true)` 召回结果概括；二者均 opt-in，未触发即 0 LLM 调用 / 0 token
...
`memory_read.retrieve_context / important_memories` 在 MCP schema 上原生暴露上述六个字段；CLI `compile / retrieve-context / important-memories / link-artifact` 也复用这些 facet。检索路径：`metadata facet 预筛 → FTS → vector supplement → rerank`，facet 过滤命中即缩小候选集，未命中不阻塞主链路。
