...
def _context_recovery_candidates(config: MemoryConfig, args: dict[str, Any]) -> list[dict[str, Any]]:
    user = _normalize_text(args.get("user")) or get_current_user(config.repo_root)
    ...
    for token, ctx in contexts.items():
        if not isinstance(ctx, dict):
            continue
        if user and str(ctx.get("user") or "").lower() != user.lower():
            continue
...
def begin_or_resolve_task(config: MemoryConfig, **kwargs: Any) -> dict[str, Any]:
    ...
    user = _normalize_text(kwargs.get("user")) or get_current_user(config.repo_root)
...
def get_task_ids_for_user(config: MemoryConfig, user: str | None) -> set[str]:
    ...
    for ctx in contexts.values():
        if not isinstance(ctx, dict):
            continue
        if str(ctx.get("user") or "").lower() != normalized_user.lower():
            continue
