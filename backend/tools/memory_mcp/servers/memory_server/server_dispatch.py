...
def _effective_retrieval_user(config: MemoryConfig, args: dict[str, Any]) -> str | None | dict[str, Any]:
    ...
    user = str(raw_user).strip() if raw_user is not None else ""
    if not user:
        task_ctx = args.get("_task_context")
        if isinstance(task_ctx, dict):
            user = str(task_ctx.get("user") or "").strip()
    if not user:
        user = get_current_user(config.repo_root).strip()
...
def _dispatch_memory_read(config: MemoryConfig, args: dict[str, Any]) -> dict[str, Any]:
    ...
    return error_result(
        "invalid_input",
        "operation must be one of: task_context, get_task_context, get, search, search_records, runtime_digest, retrieve_context, important_memories, latest_memories",
    )
