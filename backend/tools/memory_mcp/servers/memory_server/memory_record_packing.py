    ...
    for pack_index in range(1, 10000):
        ...
        try:
            ...
            with file_lock(config.repo_root, abs_path):
                ...
                if entry.record_id in _entry_ids_from_text(existing):
                    return rel_path
                if existing and len(existing) + len(payload) + 1 > max_chars:
                    continue
                if existing:
    ...
    for abs_path, rel_path in files:
        ...
        if not dry_run:
            try:
                ...
                refresh_index_if_exists(config, result)
            except OSError as exc:
