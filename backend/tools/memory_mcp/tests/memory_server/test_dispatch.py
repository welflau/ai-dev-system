...
def test_memory_read_retrieve_context_token_does_not_limit_to_token_task(repo: Path) -> None:
    ...
    current = _dispatch_tool(
        ...
        "memory_read",
        {
            ...
            "user": "alice",
            "agent_id": "copilot",
            "user_goal": "current fracture binding task",
            "max_chars": 0,
        },
    )
    other_task = memory_write_record(
        config,
        content_markdown="# SpawnRuntimeRewardDropGroup\n\nLJC fracture binding note from another agent task.\n",
        record_kind="note",
        scope="personal",
        status="validated",
        author="alice",
    ...
    bob_other_task = memory_write_record(
        config,
        content_markdown="# SpawnRuntimeRewardDropGroup Bob\n\nBob fracture binding note from another task.\n",
        record_kind="note",
        scope="personal",
        status="validated",
        author="bob",
    ...
    result = _dispatch_tool(
        ...
        {
            "operation": "retrieve_context",
            "query": "SpawnRuntimeRewardDropGroup fracture binding",
...
def test_memory_read_retrieve_context_respects_explicit_task_id(repo: Path) -> None:
    ...
    result = _dispatch_tool(
        ...
        {
            "operation": "retrieve_context",
            "query": "fracture binding task note",
            "user": "alice",
            "task_id": "task_target",
            "top_k": 10,
        },
