...
def test_record_llm_normalize_rewrites_unknown_tags_and_emits_warning(tmp_path, monkeypatch):
    ...
    result = _dispatch_memory_write(
        ...
        {
            "operation": "record",
            "content_markdown": "# Wall prefab decision\n\nLJC scene uses prefab approach.",
            "record_kind": "decision",
            "scope": "project_shared",
            "tags": ["high_value", "ljc", "wall_prefab"],
    ...
    assert suggestion["rejected_tags"] == ["ljc", "wall_prefab"]
    assert set(suggestion["final_tags"]) == {"high_value", "mcp"}
    assert suggestion["suggested_system_area"] == "ljc.wall_prefab"
    ...
    assert sorted(warn["from_tags"]) == ["high_value", "ljc", "wall_prefab"]
    assert warn["rejected_tags"] == ["ljc", "wall_prefab"]
    assert warn["system_area"] == "ljc.wall_prefab"
...
def test_record_llm_normalize_disabled_by_default_returns_invalid_input(tmp_path, monkeypatch):
    ...
    assert result["rejected_tags"] == ["ljc"]
...
def test_record_llm_unavailable_falls_back_to_invalid_input_with_suggestion(tmp_path, monkeypatch):
    ...
    assert suggestion["rejected_tags"] == ["ljc"]
