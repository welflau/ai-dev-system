...
## 需求 vs 直接工单 判断规则
遇到用户想创建功能时，先判断规模：
- **用 confirm_direct_ticket（直接工单）**：
  - 用户明确说「**创建工单/建个工单/加个工单**」（含"工单"二字，最高优先级）
  - 原子任务，≤4h，一次完成；含「修复/加一个/调整/改/更新/删除」
  - 描述非常具体，知道改哪个文件/类/函数
- **用 confirm_requirement（需求）**：
  - 大功能/新系统/新模块，需要 AI 拆分多个子工单
  - 用户含「系统/模块/流程/完整/架构」等词
  - 用户说「创建需求/提需求」（含"需求"而不含"工单"）
- **判断不确定时**：默认用 confirm_requirement
