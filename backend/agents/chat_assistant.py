...
from actions.chat.launch_ue_editor import LaunchUEEditorAction             # 启动 UE Editor
...
class ChatAssistantAgent(BaseAgent):
    ...
    action_classes = [
        ...
        LaunchUEEditorAction,          # 启动 UE Editor（仅 engine:ue5/ue4）
...
请用中文，200字以内。"""
    ...
    async def _build_system_prompt(self, project: dict, context: dict, history_len: int = 0) -> str:
        ...
        # A-3: Memory 注入（使用 BaseAgent.get_memory_prompt，4 類型 MEMORY.md 格式）
        memory_section = ""
        project_id = project.get("id", "")
        if project_id:
            mem_text = await self.get_memory_prompt(project_id, limit=5)
            if mem_text:
                memory_section = "\n" + mem_text + "\n"
...
**以下操作完成后，必须立即自动触发 UBT 编译，不需要等用户提醒：**
- 修改了 `.uproject` 的 `EngineAssociation`（切换引擎版本或路径）
- 执行了 `GenerateProjectFiles`（重新生成了 VS 项目文件）
- 修改了 `Build.cs` / `Target.cs` / `*.h` 等 C++ 源文件
- 安装或删除了插件（`Plugins/` 目录变更）
...
**不要**等用户说"编译一下"才编译——这些操作后项目必然需要重新编译，应主动执行。"""
        ...
        return f"""{rules_section}**【语言要求】无论用户用什么语言提问，你的所有回复、思考过程、工具调用说明，全部必须使用中文。严禁出现英文思考段落。**
...
""" if _skills_index else ""
        ...
        return f"""{global_rules_section}**【语言要求】无论用户用什么语言提问，你的所有回复、思考过程，全部必须使用中文。严禁英文思考段落。**

你是 AI 自动开发系统的全局助手，当前用户在**项目列表页**，**还没有进入任何具体项目**。

## ⚠️ 操作协议（必须严格遵守）

当你需要执行以下操作时，**不要描述意图，直接在回复中嵌入 ACTION 标签**：

### 创建项目草稿（confirm_project）
示例格式：
[ACTION:confirm_project]
名称: FPSCombat
traits: platform:desktop, category:game, engine:ue5
mode: auto
git_remote_url: （留空或填写）
description: UE5 FPS 射击游戏
[/ACTION]
...
**⚠️ 重要：**
- 只要信息足够（platform + category + engine 明确），**立即输出 ACTION 标签**，不要先问用户确认
- ACTION 标签可以和正常文字混合在同一条回复里
- `git_remote_url` 如果不知道就留空
- 不要说"我调用了工具"或"正在创建"——直接输出标签就会触发
