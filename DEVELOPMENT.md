...
AI 自动开发系统是一个从自然语言需求到可运行软件的端到端自动化开发平台。用户提交需求描述后，系统自动完成需求分析、架构设计、代码生成、测试验证、代码审查和部署配置全流程。
...
├── backend/                      # 后端代码
│   ├── agents/                   # AI Agent 实现
│   │   ├── __init__.py           # 导出全部 Agent
│   │   ├── base.py               # BaseAgent 抽象基类
│   │   ├── product.py            # 原始 ProductAgent (async, 通过 Adapter 使用)
│   │   ├── dev.py                # DevAgent (代码生成, 14 种任务 + LLM)
│   │   ├── architect.py          # ArchitectAgent (架构设计, 7 种方案 + LLM)
│   │   ├── test_agent.py         # TestAgent (测试生成, LLM + 模板)
│   │   ├── review_agent.py       # ReviewAgent (代码审查, 10 种规则 + LLM)
│   │   └── deploy_agent.py       # DeployAgent (部署配置, Docker/CI/Nginx)
│   ├── models/                   # 数据模型
...
│   │   ├── test_agent.py         # Agent 模板生成测试
│   │   └── test_new_agents.py    # ReviewAgent + DeployAgent + ProductAdapter 测试 (24)
...
```
用户需求 → FastAPI → Orchestrator → TaskDecomposer → DbStateManager
                                        ↓
                                    Agent 池 (上下文传递)
              ┌──────────┬──────────┬──────────┬──────────┬──────────┐
...
TestAgent → 测试代码（参考已有代码生成针对性测试）
                ↓ (review_context)
ReviewAgent → 代码审查报告（10 种规则 + LLM 智能分析）
                ↓ (deploy_context)
DeployAgent → 部署配置（Dockerfile + docker-compose + CI/CD + Nginx）
```
...
| ReviewAgent | agents/review_agent.py | 代码审查 (10 种规则 + 评分 A~F) | ✅ | ✅ |
| DeployAgent | agents/deploy_agent.py | Docker + CI/CD + Nginx 部署配置 | ✅ | ✅ |
...
### 6. Pipeline 看板 (v0.6.0)

前端 Pipeline 看板视图：
- 横向 Stage 卡片（需求→架构→开发→测试→部署）+ 连接箭头
- 阶段状态可视化：completed(绿) / running(蓝脉冲) / failed(红) / pending(灰)
- 步骤点击打开日志抽屉（VS Code 风格暗色终端）
- task_logs 表 + started_at/completed_at/duration_seconds 时间追踪
...
| GET | /api/projects/{id}/download | 项目 ZIP 打包下载 |
| GET | /api/projects/{id}/tasks/{task_id}/logs | 查询任务执行日志 |
...
| 项目详情 | project-detail | Pipeline 看板、阶段分组任务列表、统计摘要、文件浏览、代码高亮预览、SSE 实时更新、日志抽屉 |
| 工具列表 | tools | 已注册开发工具 |
...
python -m pytest tests/test_new_agents.py -v      # Review/Deploy/Product 测试 (24)
```
...
| 版本 | 日期 | 主要功能 |
|------|------|---------|
| v0.1.0 | 2026-03-23 | 基础框架 + 工具系统 |
| v0.2.0 | 2026-03-23 | DevAgent + ArchitectAgent + 前端看板 |
| v0.3.0 | 2026-03-23 | SQLite 持久化 + 一键执行 + 文件浏览 |
| v0.4.0 | 2026-03-23 | LLM 智能引擎（任务分解 + 代码生成 + 架构设计）|
| v0.5.0 | 2026-03-23 | Agent 上下文传递 + TestAgent + ProductAgent + SSE + 语法高亮 |
| **v0.6.0** | 2026-03-23 | ReviewAgent + DeployAgent + Pipeline 看板 + ZIP 打包 + 日志面板 (92 个测试) |
